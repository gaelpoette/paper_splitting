...
\begin{remark}
  \label{stopping}
Something important which seems to, to our knowledge, not have been noticed in the litterature is that 
  expression \eqref{gni} is consistent with $F=\sum_{i=1}^P F_i$ in the sense we have $\forall i\in\{1,...,m\}$ 
  \begin{equation}
...
global minimums of $\R$. A faster convergence rate is consequently numerically observed.  
  At epoch $n=2000$, for which stationarity is reached (in fact, the stopping criterion \eqref{stopping_criterion} of remark \ref{stopping} is activated at $n\approx 40 - 60 \ll 2000$ depending on the
