...
\newG{Ici dire qu'on peut facilement modifier IG SARAH  pour qu'il verifie les 5 premiers et ensuite on ira vers la part II. }
...
\begin{remark}
	...
	\begin{equation*}
		...
		\end{pmatrix}.
	\end{equation*}
        Figure \ref{mom_ex1_ex2} revisits the benchmarks \exOne and \exTwo but with Momentum. 
        ...
        The distributions are displayed at two epochs: for epoch $n=20$, stationarity is not yet reached with Momentum, for epoch $n=2000$, it is.  
        ...
        is qualitatively better than the results obtained with GD. 
        But at epoch $n=20$, the results are not yet converged and  
\begin{figure}[!h]
	\centering
	...
	\label{mom_ex1_ex2}
\end{figure}
  on the last line of figure \ref{mom_ex1_ex2}, at epoch $n=2000$, stationarity is reached and Momentum captures respectively
  $x=0.40533333$ instead of the unique global minimum $0.4$ of \exOne. 
  and $x=-0.72457792$ instead of $0.71$ of the global minimum of \exTwo. 
  In other words, the appearance of artificial minima strongly related to the choice of the learning rate also occurs for other optimisers and is not 
  only for GD. 
...
global minima of $\R$. 
  At epoch $n=2000$, for which stationarity is reached (the stopping criterion is activated at $n\approx 40 - 60$), th Speth-Momentum algorithm recovers the 
  position of the minima with a very good accuracy. 

	The application of one of our extension of Speth's splitting above gives a fundamentally different update of STORM (variance reduction on Momentum). Therefore, the balanced splitting point of
        view might lead to some optimizers like SAG or RR-SARAH but it is by construction different of the variance reduction technique. The same difference appears for the variance reduction versions related to Adam: see Adam$^+$ \cite{adam_variance1} for example.
...
\newG{TODO: stopping criterion!}

\newG{TODO: time step <--> learning rate}

In this section, we suggest applying the new previously described splitting to the 
...
epochs/batches.  
\ \\ \ \\
TODO Here, tuning of the time step for speth (figure \ref{SPETH_exs}). Figure \ref{SGD_exs} is obtained with the same lr as the one for speth of figure \ref{SPETH_exs}. 
Figure \ref{SGD_exs_001} is obtained with SGD with a time step $100$ times smaller than the one used for Speth. 
Blbala todo
...
\begin{figure}[h!]
	...
	\caption{RRGD dt 100 fois plus petit que speth}
	\label{SGD_exs_001}
\end{figure}
