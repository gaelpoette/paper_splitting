...
\begin{abstract}
	Mini batch optimizers are at the center of neural network training. 
        ...
        new mini-batch optimizers (and even allow revisiting some known ones as special cases of the suggested strategy, namely IG-SARAH).
...
rate. In broad outline, the aim is to conceive methods that ensure that the variance does not\footnote{\newG{does ou does not? Ambigu ici... Et $\to$ c'est quand $m\to \infty$ je
suppose}} vanish asymptotically:
\begin{equation*}
...
Something important which seems, to our knowledge, to not have been noticed in the litterature is that 
  expression \eqref{gni} is consistent with $F=\sum_{i=1}^P F_i$ in the sense that $\forall i\in\{1,...,m\}$ 
  \begin{equation}
...
\end{figure}
  ...
  On the other hand, figure \ref{speth_mom_ex1_ex2} presents the results obtained with Speth-Momentum, in exactly the same conditions as the ones of figure \ref{mom_ex1_ex2}. 
\begin{figure}[!h]
	\centering
...
points of $F$). Finally, we have suggested a balanced splitting formula which differ from the classical ones (and encompass some classical ones) %by their memory complexity and 
...
\newG{JE PENSE QU'IL FAUT METTRE CETTE PARTIE DANS PART II}
~~\\
...
satisfies condition 5 relative to the existence of a stopping criterion \eqref{stopping_criterion} (see remark \ref{stopping}) which does not need a full batch estimation and condition 1 related to fact of
...
\begin{itemize}
	...
	\item To understand this undesirable behavior, we suggest a new point of view on RRGD reinterpreted as a splitting scheme applied on the gradient flow. This turns the classical approaches upside down: it consists in suggesting a new continuous system (a stochastic ODE) and then to discretize it with an explicit Euler scheme. This perspective makes it possible to explain the appearance of "artificial" steady states that are related to the unbalancing of the splitting: the steady states may depend on the learning rate. With the classical approach based on the brownian motion, it is impossible to explain the source of such a singular behavior (if the diffusion is constant, the stationary distribution is the Gibbs distribution).
         \item A new family of algorithm is then proposed to overcome the former issue. Based on a well-balanced strategy, it avoids to capture artificial minimums due to the batch
           ...
           criterion which doesn't require to compute full batch gradients. The mini-batch optimizer IG-SARAH of the litterature is part of this family. 
\end{itemize}
