...
\newG{ICI FAIRE REMARQUER LA CONSISTENCE AVEC LE GRADIENT EN INTRODUCTION DU STOPPING CRITERION}.
...
have stable resolutions. By tuning $n_{max}$, we mean ensuring stationarity of the distributions.
...
we will see in part II of this paper that adaptivity is necessary in order to capture this stiff global minimums.  
...
artificial minimums are captured. These artificial minimums can be quite fare from the true ones, see \exFour, \exSeven, \exHeight.    
Very often, the distributions are not veru peaked on the vicinities of the minimums.
On the last benchmarks, \exHeight, neither the local nor the global minimums are captured. 
...
On \exThree, the local and global minimums are now in the support of the distribution, but their probabilities of occurrence remain low.
