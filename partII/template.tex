...
with respect to the non-interpolatory condition: in part I of this paper, care has been taken to ensure that (for both \eqref{RAG_scheme} and \eqref{RAGL_splitting}) if a limit $y_\infty$ for $y_{n+i/m}$ exists as $n\rightarrow \infty$, then it satisfies $F(y_\infty)=0$. 
...
\newG{To ease the reading, the complete presentation of some algorithms is sent back to appendix \ref{annexe_algo}, especially the backtrackings, the initializations, the RAGL
...
As stated in the previous section, we now would like to reduce the memory cost of the previous adaptive well-balanced mini-batch optimizer. 
...
The RAGL-prototype algorithm we first suggest, explicited in algorithm \ref{algo_RAGL_proto}, is an adaptive version of the splitting \eqref{RAGL_splitting}. 
It is of complexity $3\times N$ and can also be viewed as a deterministic adaptive version of RR-SARAH. 

\newG{ICI, IL FAUT EXPLIQUER POURQUOI IL ETS PAS ADAPTÉ AU PB STIFF parce qu'on fait ref a cette section plus tard...}\\
...
\begin{itemize}
  ...
  \item On pourrait rendre le truc plus progressif avec 
    ...
    \end{itemize}
    A reussir a introduire: si on avait acces aux $L_i$, on aurait moins besoin d'un algo adaptif. On pourrait avoir acces a un nouvel algo a pas constant... Mais comme on a un pas
...
\newG{Question dans les algo:
\begin{itemize}
  \item encapsuler les linesearches? Pourrait ne pas suffire pour bien mettre en evidence les differences de manière progressive
