...
\end{equation*}
where $(x_p,y_p)_{1\leq p \leq P}$ are the data of the problems, $u(\theta,x)$ a parametrized model (for example a neural network) and $\loss$ a loss function measuring the distance between predictions $u(\theta,x)$ and real data. 
...
Then at every iteration, the algorithm has only access to one function $\R_i$.
...
\end{equation}
where $\eta$ is the learning rate and $F$ is the flow: for Gradient Descent (GD), $y=\theta$ and $F(y)=-\nR(\theta)$, for Momentum, $y=(\theta,v)^t$ and $F(y)=(-v, -\bbetone v +
\nR(\theta))^t$. All classical optimizers can be put into the previous expression \eqref{fb_opt}.  
...
\end{multline}
for all $i \in \{1,\dots,m\}$. The previous algorithm with reduced memory cost remains balanced in the sense it keeps the good properties of mini-batch update \eqref{RAG_scheme}
...
All in all, in part I of this paper, it has been numerically emphasized on analytical one dimensional benchmarks that the new family of mini-batch optimizers can efficiently
\begin{enumerate}
	...
	\item The algorithms never use the full-batch gradient, at any stage, due to storage requirement. 
	...
	\item And all the new algorithms of the family come with a \textbf{stopping criterion} that ensures that the full-batch gradient is smaller than a fixed arbitrary
...
\item But the well-balanced optimizers of part I still need the tedious tuning of the (constant) learning rate $\eta$. 
\end{enumerate}
Building an adaptative strategy on top of the well-balanced mini-batch
          optimizers of part I is the purpose of this second part. 
...
current batch and estimate its local Lipschitz
constant to combine them into an original well-balanced adaptive family of algorithms. 
In section \ref{section_results}, the newly developped algorithms are tested on analytical benchmarks and on classical machine learning tasks. They are compared with
...
\label{adapt}
~~\\
All the mini-batch optimizers mentioned above \newG{(sans doute pas assez ICI...)} use decreasing or constant learning rate, so tuning is generally necessary to make the methods converge. In fact, there are very few works about adaptive learning rate mini-batch optimizers (algorithms that do not need tuning):
\begin{itemize}
...
\end{itemize}
In the next section, we present our adaptive strategy: it is also based on linesearch relative to the Armijo rule but it is not only applied to the current batch but on a slight
modification of it in which the aggregator plays a central role. \newG{GD mais rien sur le fait que des efforts ont été faits pour généraliser Armijo à d'autres trucs... Peut etre
à mettre ailleurs mais à mettre qq part} 


\section{Adaptive well-balanced optimizers}
\label{section_optimizers}

In this section, we suggest a way to make the formulas \eqref{RAG_scheme}, \eqref{RAGL_splitting} %and \eqref{RAGL2_splitting} 
...
To ease the reading, the complete presentation of some algorithms is sent back to appendix \ref{annexe_algo}, especially the backtrackings, the initializations, the RAGL prototype and RAGL.
...
replaced by a Lyapunov function relative to $F$, see \cite{Lyap_Theory_Bilel,Bilel_thesis,bakharat,..,..}). But the advantage of focusing on a well-balanced adaptative mini-batch version of GD is that
the only hyperparameter to tune is the learning rate (whereas Momentum, RMSProp, Adam etc. also need the tuning of several other coefficients, see \cite{fdqsfq}). 
...
Note that having access to $\eta$ fulfilling the above inequality also gives access to an additional information: indeed, according to the descent lemma (proposition 1 of \cite{Lyap_Theory_Bilel}), 
\begin{equation}
  \label{L_i}
...
batch $i$ will be available when dealing with the other ones, even if slight out of date.  


Now, the risk is that even if $\eta$ satisfies \eqref{armijo_batch1}, the chances is satisfies the Armijo condition on batch $j>i$ are poor, especially in the non-interpolatory
case. In such case, we can easily imagine some situations in which the gradient of batch $i$ is far from the one of the full batch. 
...
\end{equation*}
Now assume that from the previous backtrackings, along the previous batches, we have an idea of which batch is the stiffest in the vicinity of $\theta$ (thanks to \eqref{L_i}). Let us denote by $i_{max}$
is the index of the stiffest batch (largest Lipschitz constants of the gradients).
\newG{ICI, UN PEU TROP RAIDE}
Then, we remplace $\R$ by $\R_{i_{max}}$. Indeed, if we proceed to this linesearch on a little steep batch, we would underestimate the stiffness of $\R$: with the "worst" batch this can not happen. The inequality finally becomes:
\begin{equation*}
	\R_{i_{max}}(\theta-\eta g) - \R_{i_{max}}(\theta) \leq -\lambda \eta g_{i_{max}}\cdot g.
\end{equation*}
In a nutshell, the previous step is only called on the stiffest batch if the dot product satisfies $g_i \cdot g>0$ and if $g_i \cdot g \leq \|g\|^2$. 
...
the
numerical experiments of section \ref{section_results} will tend to show that a better memory cost/preformance compromise can be made with the RAGL optimizer of section \ref{RAGL}
...
\begin{enumerate}
	...
	$\nR_1,$ $\dots, \nR_m$ thanks to the linesearch without interaction. A possible Lipschitz constant for $\nR$ is then obtained by $L=L_1+\dots+L_m$. The learning rate is then computed as $2(1-\lambda)L^{-1}$ according to proposition 1 of \cite{Lyap_Theory_Bilel} and a GD step is done. Notice that for the rest of the epochs we define the learning rate as $2L^{-1}$ (neglect the factor $1-\lambda$) because this choice enables empirically to save time.
	...
	\item This maximum is exploited for the memory effect (see \cite{Lyap_Theory_Bilel,Bilel_ICML} and chapter 2 of \cite{Bilel_thesis}) at line 29. Here, the learning rate from
          ...
          theorem 5.1 of \cite{Bilel_ICML} and theorem 2.5 of \cite{Bilel_thesis}. The idea is to create a recurrent relation between the admissible learning rates of two successive
...
\subsection{\newG{The RAGL-prototype optimizer: {\it Rebalanced Aggregated Gradient Light prototype}}}
\label{RAGL_proto}
\newG{ICI, IL FAUT EXPLIQUER POURQUOI IL ETS PAS ADAPTÉ AU PB STIFF...}
...
~~\\
As explained in subsection \ref{RAGL_proto}, RAGL-prototype is not adapted to deal with stiff
...
\paragraph{One dimension benchmark (described in part I)}
~~\\
