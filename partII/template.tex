...
All in all, in \cite{partI}, it has been numerically emphasized on analytical one dimensional benchmarks that the new family of mini-batch optimizers can efficiently
\begin{enumerate}
	\item {avoid assumptions on the batch distribution}, 
          %Firstly, the bounded variance hypothesis is not reasonable 
	...
	\item achieve an arbitrary precision (concerning the gradient) without requiring too drastic decreasing learning rates. \\ \ \\
          Moreover: \\
          %This is only verified by variance reduction methods. 
	\item The algorithms never use the full-batch gradient, at any stage. 
          %The ones that fit into this category are: SGD, RRGD, IG, IAG, SAG, ZERO-SARAH, STORM, SCSG et RR-SARAH.
        \item The reduced memory version do not store $m$ gradients (only $3$). 
          %mFor example, the optimizers that do not need such a storage are:  SGD, RRGD, IG, SVRG, SNVRG, SPIDER, SARAH, SSRGD, PAGE, SCSG et RR-SARAH.
...
$\forall i\in\{1,...,m\}$ 
  ...
  \epsilon$, with $\epsilon$ prescribed by the user. 
          %All the mentionned optimizers are stopped when a maximal number of epochs is achieved (to the best of our knowledge).
\end{enumerate}

Before turning to adaptivity, let us illustrate on a genuinely two-dimensional, stiff benchmark why balancing is a prerequisite for {\em any} mini-batch/splitting optimizer, and not only for eS-GD. Let \polyFive\ (introduced in \cite{Bilel}, see also Section~\ref{RAGL} for its definition) denote this benchmark and let us recall that {\em sensitivity maps} (precisely defined in Section~\ref{test_analytic}) color every initialization $\theta_0$ according to the critical point (or the divergence) its trajectory converges to. \\
Figures~\ref{fig_speth_motivation_a} and \ref{fig_speth_motivation_b} compare, for six full-batch optimizers -- GD, Momentum, Adam, RMSProp, Lion and AdamW, whose Lyapunov functions are recalled in \cite{Lyap_Theory_Bilel,Bilel_thesis} -- three variants: their full-batch version (left column), a naive mini-batch/splitting version obtained with $m=2$ non-interpolating batches by simply {\em not} balancing update \eqref{RAG_scheme} (\emph{i.e.} $c_n\equiv 0$, middle column), and their extended Speth-balanced counterpart (eS-GD, eS-Momentum, eS-Adam, eS-RMSProp, eS-Lion, eS-AdamW, right column) with the exact same $m=2$ batches. For a given row, the three columns share the very same constant learning rate ($\eta=10^{-2}$ for GD, Momentum and RMSProp, $\eta=10^{-3}$ for Adam, Lion and AdamW): the value tuned for the full-batch algorithm is simply reused, without any retuning, for both mini-batch variants. \\
...
GD is the instructive counter-example: at $\eta=10^{-2}$, the full-batch algorithm is {\em already} almost entirely divergent on this stiff benchmark ($96.3\%$), and balancing cannot rescue what is broken upstream -- the Speth-balanced map ($98.8\%$ divergent) is no better than the unbalanced one ($95.2\%$). This is expected: balancing only guarantees that {\em if} the mini-batch/splitting scheme converges, it converges to a critical point of the full-batch objective (Theorem~1 of \cite{partI}); it says nothing if the underlying constant learning rate is simply not admissible in the first place. \\
This experiment extends, to genuinely two-dimensional and non-GD optimizers, the message of \cite{partI}: balancing the mini-batch/splitting update is what allows recovering the qualitative behavior of its full-batch counterpart, whenever the latter is itself well-behaved. But it also makes plain what is still missing on both counts: here, the learning rate was tuned {\em once}, on the full-batch problem, and merely reused for the two mini-batch variants -- information that is, in general, unavailable in a genuine mini-batch setting (typically because the full-batch optimizer cannot even be run, due to the very memory constraints that motivate mini-batching in the first place) -- and the GD row shows that an untuned constant learning rate can defeat balancing altogether, whether mini-batch or full-batch.
