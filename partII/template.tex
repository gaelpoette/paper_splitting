...
with respect to the non-interpolatory condition: it is easy checking that (for both \eqref{RAG_scheme} and \eqref{RAGL_splitting}) if a limit $y_\infty$ for $y_{n+i/m}$ exists as $n\rightarrow \infty$, then it satisfies $F(y_\infty)=0$. 
...
la sensitivity map de RAG, RAGL-proto et RAGL sur PolyAllStiff (le cas du tableau). On pourrait rendre le truc plus progressif. }
