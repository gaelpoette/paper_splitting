...
\begin{enumerate}
	...
	\item The reduced memory version do not store $m$ gradients. 
...
$\forall i\in\{1,...,m\}$ 
  ...
  \epsilon$, with $\epsilon$ prescribed by the user, and this without resorting to a full batch estimation of $\nR$. This will be numerically verified in section \ref{num_res}. 
          %All the mentionned optimizers are stopped when a maximal number of epochs is achieved (to the best of our knowledge). 
	\item But... finally want an \textbf{adaptive time step method} since the main goal of this paper is to avoid tuning hyperparameters. In this case, all the methods that do not necessitate a growth condition or a precise knowledge of the noise make the batch size vary with a risk to exceed the memory. Let us note that such an approach can generate strong slow down due to dynamic memory access. Let us also note that in practice the batch size is not meant to be a hyperparameter to tune since it is given by the quotient between the data size and the RAM size. The only optimizers that overcome this difficulty are the CMA algorithms. 
