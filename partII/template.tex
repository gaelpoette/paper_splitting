...
for Momentum, $y=(\theta,v)^t$ and $F(y)=(-v, -\bbetone v +
\nR(\theta))^t$. 
All classical optimizers can be put into the previous expression \eqref{fb_opt}, see \cite{papier_ou_il_ecrive_tout_de_maniere_generale}.  
...
\label{adapt}
~~\\
All the mini-batch optimizers mentioned above \newG{(je crois qu'ils ne sont plus mentionned above... Retoruver lesquels)} use decreasing or constant learning rate, so tuning is generally necessary to make the methods converge. In fact, there are very few works about adaptive learning rate mini-batch optimizers (algorithms that do not need tuning):
\begin{itemize}
...
\end{equation}
\newG{ICI, UN PEU TROP RAIDE... JE FAIS UNE TENTATIVE...}\\
\newG{Now, assume that for each batch, $\eta$ satisfies 
\begin{equation}
  \label{ok_i}
	\R_i(\theta-\eta g) - \R_i(\theta) \leq -\lambda \eta \|g\|^2 \leq -\lambda \eta g_i\cdot g,
\end{equation}
then inequality \eqref{ok} is fulfilled.  
But for computational cost reasons, we want to avoid performing this additional linesearch for each batch. 
Now, assume that from the previous backtrackings (the one depicted in the previous paragraphs), 
...
\end{equation*}
it will satisfy \eqref{ok_i} (by definition of being the stiffest) hence \eqref{ok}.} 
...
The previously described algorithm is consequently adaptive and relies on updating the weights at each batches according to a well-balanced aggregator. 
...
we will see that the algorithm is very efficient in practice. Due to its characteristics, it will be particularly adapted in the case in which the data can not fit into memory
but the architecture can 

\newG{ICI, J'AIMERAIS BIEN UNE BELLE PHRASE QUI MONTRE QUE C'EST PAS SI  GRAVE: SI ON A BEOSIN DE LA MEMOIRE POUR L'ARCHI ET LES M GRADIENTS, PAS GRAVE, ON MET MOINS EN MÉMOIRE ET
L'ALGO ENCAISSE BIEN LES DESEQUILIBRE DE BATCHS. Mais au cas ou blabla... }


\subsection{\newG{The RAGL-prototype optimizer: {\it Rebalanced Aggregated Gradient Light prototype}}}
\label{RAGL_proto}

As explained in the previous section, we now would like to reduce the memory cost the previous adaptive well-balanced mini-batch optimizer. 

The procedure used for optimizer RR-SARAH is applicable to our problem...
