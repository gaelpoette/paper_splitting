...
\nR(\theta))^t$. 
All classical optimizers can be put into the previous expression \eqref{fb_opt}, see \cite{Barakat_bianchi}.  
...
In section \ref{section_results}, the newly developed algorithms are tested on analytical benchmarks and on classical machine learning tasks. They are compared with
...
($\approx 10^{-16}$ for doubles in IEEE754). To ease the reading, we also denote by $g$ the update vector for one iteration: for example for the splitting \eqref{RAG_scheme}, it is
...
\newG{ICI, IL FAUT EXPLIQUER POURQUOI IL ETS PAS ADAPTÉ AU PB STIFF parce qu'on fait ref a cette section plus tarf...}
