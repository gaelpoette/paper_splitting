...
%
% 2) enlever cette intro jusqu'à ``Adaptive learning rates''
%
...
\end{equation*}
in which $\R$ has a particular sum-structure on all the labels ($x_p,y_p)_{p\in\{1,...,P\}}$ of the problem:
\begin{equation*}
	\R(\theta) = \displaystyle{\sum_{p=1}^{P} \loss(u(\theta,x_p), y_p)}.
\end{equation*}
In the above expression, %$(x_p,y_p)_{1\leq p \leq P}$ are the data of the problems, 
$u(\theta,x)$ is a parametrized model (for example a neural network) and $\loss$ is a loss function measuring the distance between predictions $(u(\theta,x_p))_{p\in\{1,...,P\}}$
and real data $(y_p)_{p\in\{1,...,P\}}$. 
%In practice, the memory complexity is proportional to the number of data $P$ and $P$ is big. Therefore, the memory cost exceeds the RAM storage. 
...
In paper \cite{partI}, 
 a new family of mini-batch optimizers has been put forward. 
...
%Based on the work of Speth \cite{rebalanced_splitting}, who designed a balanced splitting for stiff convection-diffusion operators, a new family of mini-batch optimizers has been
%presented.  
%The material of \cite{partI} is in fact an extension of the work of \cite{rebalanced_splitting} and not a direct application.
In \cite{partI}, the authors identified an analogy between handling non-interpolatory batches $\R_i$
 and operator splitting for stiff, non-commutative operators \cite{rebalanced_splitting}. Building on the work of Speth \cite{rebalanced_splitting}, who devised a balanced splitting scheme for stiff convection-diffusion operators, a new family of mini-batch optimizers is introduced. The approach developed in \cite{partI} should be understood as an extension of \cite{rebalanced_splitting} rather than a direct application of it:
any full-batch optimizer of interest can be written as
\begin{equation}
	y_{n+1} = y_n+\eta F(y_n),
	\label{fb_opt}
\end{equation}
where $\eta$ is the learning rate and $F$ is the flow. For GD, $y=\theta$ and $F(y)=-\nR(\theta)$. Momentum, RMSProp, Adam,... fit into the same form, see \cite{Barakat_bianchi}. 
Splitting $F=\sum_{i=1}^m F_i$ with $F_i=-\nR_i$ and naively composing the $F_i$ (\emph{e.g.} \`a la Lie-Trotter) is exactly what RRGD/IG do, and it is {\em unbalanced}: as recalled above, the resulting steady state generally depends on $\eta$ and need not be a critical point of $\R$. Following Speth et al. \cite{rebalanced_splitting}, \cite{partI} cures this by sharing a correction vector between the operators: for $m=2$, $F_1^*:=F_1+c$ and $F_2^*:=F_2-c$ both admit the same steady state $y_\infty$ as $F=F_1+F_2$ as soon as $c=\frac12[F_2(y_\infty)-F_1(y_\infty)]$; since $y_\infty$ is unknown, $c$ is instead re-estimated {\em on the fly}, using only already available evaluations of the $F_i$. Generalized to $m$ operators, this balancing yields the mini-batch update put forward in \cite{partI}:
\begin{equation}
  \begin{array}{l}
  ...
  \end{array}
	\label{RAG_scheme}
\end{equation}
for $i\in\{1,\dots,m\}$: the aggregator $g_n^i$ mixes fresh and delayed evaluations of every batch so that, whenever a limit $y_\infty$ exists, $F(y_\infty)=0$, without requiring the interpolation condition (Theorem~1 of \cite{partI}). Implementing \eqref{RAG_scheme} as such requires storing all $m$ evaluations $F_j\left(y_{n-1+(j-1)/m}\right)$ (memory cost $mN$). A lighter, $3N$ variant instead updates a single running aggregator $g_n=\sum_{j=1}^m F_j\left(y_{n-1+(j-1)/m}\right)$ by swapping, one batch at a time, its stale contribution for a fresh one:
\begin{equation}
	y_{n+i/m} = y_{n+(i-1)/m} + \eta
	\begin{cases}
		g_n+\displaystyle\sum_{j=1}^i \left[F_j\left(y_{n+(j-1)/m}\right)-F_j\left(y_{n+(j-2)/m}\right)\right], & 1\leq i<m,\\
		g_{n+1}, & i=m,
	\end{cases}
	\label{RAGL_splitting}
\end{equation}
which remains balanced in the same sense as \eqref{RAG_scheme}. Both aggregators also share a useful consistency property (\cite{partI}): for all $i\in\{1,\dots,m\}$,
\begin{equation}
	g_n^i = F(y_n) +\mathcal{O}(\eta),
	\label{stopping_criterion}
\end{equation}
so that $g_n^i$ can serve, throughout the paper, to build a stopping criterion without ever evaluating $F$ on the full batch.

Figures~\ref{fig_speth_motivation_a} and \ref{fig_speth_motivation_b} compare, for six full-batch optimizers -- GD, Momentum, Adam, RMSProp, Lion and AdamW
%, whose Lyapunov functions are recalled in \cite{Lyap_Theory_Bilel,Bilel_thesis} 
-- three variants on a simple benchmark introduced in \cite{Lyap_Theory_Bilel,Bilel_thesis}: their full-batch version (left column), a naive mini-batch/splitting version obtained with $m=2$ non-interpolating batches by simply {\em not} balancing update \eqref{RAG_scheme} (\emph{i.e.} $c_n\equiv 0$, middle column), and their extended Speth-balanced counterpart (eS-GD, eS-Momentum, eS-Adam, eS-RMSProp, eS-Lion, eS-AdamW, right column) with the exact same $m=2$ batches. For a given row, the three columns share the very same constant learning rate ($\eta=10^{-2}$ for GD, Momentum and RMSProp, $\eta=10^{-3}$ for Adam, Lion and AdamW): the value tuned for the full-batch algorithm is simply reused, without any retuning, for both mini-batch variants. \\
The conclusion is unambiguous for five of the six optimizers. For Adam, RMSProp and AdamW, balancing alone suffices to essentially recover the full-batch map with the {\em same} learning rate, whereas the unbalanced mini-batch version is considerably degraded by large spurious divergent areas (in red), absent from the full-batch map. Quantitatively (over $28000$ initializations), the proportion of divergent trajectories goes from $0\%$ (full-batch) to $42.7\%$ (unbalanced) back down to $0\%$ (Speth-balanced) for Adam, from $1.6\%$ to $30.7\%$ back down to $0\%$ for RMSProp, and from $10.5\%$ to $49.4\%$ back down to $10.6\%$ for AdamW, whose recovered map is visually near-indistinguishable from the full-batch one. Momentum and Lion are more challenging: Momentum's full-batch map is already largely divergent ($68.1\%$) on this stiff benchmark, a signature of the well-documented Lyapunov instabilities of Momentum \cite{Bilel,Bilel_thesis}, yet balancing still consistently reduces the divergent area compared to the unbalanced splitting ($72.0\%$ against $74.9\%$) and restores a map whose global shape is visually close to the full-batch one; Lion goes from $12.8\%$ (full-batch) to $80.8\%$ (unbalanced) down to $20.2\%$ (Speth-balanced) -- two orders of magnitude closer to the full-batch value than the unbalanced splitting, and with the same diagonal split recovered, even though not pixel-perfect. \\
GD is the instructive counter-example: at $\eta=10^{-2}$, the full-batch algorithm is {\em already} almost entirely divergent on this stiff benchmark ($96.3\%$), and balancing
cannot rescue what is broken upstream. The Speth-balanced map ($98.8\%$ divergent) is no better than the unbalanced one ($95.2\%$). This is expected: balancing only guarantees
that {\em if} the mini-batch/splitting scheme converges, it converges to a critical point of the full-batch objective (Theorem~1 of \cite{partI}): in other words, it says nothing if the underlying constant learning rate is simply not admissible in the first place. \\
This experiment extends, to genuinely two-dimensional and non-GD optimizers, the message of \cite{partI}: balancing the mini-batch/splitting update is what allows recovering the
qualitative behavior of its full-batch counterpart, whenever the latter is itself well-behaved. But it also makes plain what is still missing on both counts: here, the learning
rate was tuned {\em once}, on the full-batch problem, and merely reused for the two mini-batch variants 
(information that is, in general, unavailable in a genuine mini-batch setting typically because the full-batch optimizer cannot even be run, due to the very memory constraints that motivate mini-batching in the first place) and the GD row shows that an untuned constant learning rate can defeat balancing altogether, whether mini-batch or full-batch.

\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{chapitre5_img/speth_motivation_poly5_a.png}
	\caption{Sensitivity maps on \polyFive\ for GD, Momentum and Adam (rows): full-batch (left), unbalanced mini-batch/splitting with $m=2$ (middle), extended Speth-balanced with $m=2$ (right). For a given row, all three columns share the exact same constant learning rate. Green: global minimum, purple: local minimum, orange: degenerate saddle point, red: divergence.}
	\label{fig_speth_motivation_a}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{chapitre5_img/speth_motivation_poly5_b.png}
	\caption{Sensitivity maps on \polyFive\ for RMSProp, Lion and AdamW (rows), continuing Figure~\ref{fig_speth_motivation_a}: full-batch (left), unbalanced mini-batch/splitting with $m=2$ (middle), extended Speth-balanced with $m=2$ (right). Same color code as Figure~\ref{fig_speth_motivation_a}.}
	\label{fig_speth_motivation_b}
\end{figure}

As just illustrated, the well-balanced optimizers of \cite{partI} still need the tedious tuning of the (constant) learning rate $\eta$, an information that, as discussed above, is generally out of reach in a genuine mini-batch setting.
On the other hand, in a {\em full-batch} context, a discrete counterpart of the continuous Lyapunov stability condition has recently been used to select the learning rate $\eta$ automatically, with theoretical guarantees, generalizing the classical Armijo backtracking to a wide class of full-batch optimizers \cite{Lyap_Theory_Bilel,Bilel_thesis,Bilel_ICML}. In a nutshell, given a full-batch optimizer of the form \eqref{fb_opt} and an associated Lyapunov function $V$ (for GD, $V=\R$; for Momentum, RMSProp, Adam, see \cite{Lyap_Theory_Bilel,Bilel_thesis}), the learning rate $\eta_n$ is chosen, at every iteration, as an $\eta_n$ satisfying the discrete dissipation inequality
\begin{equation}
	V(y_n+\eta_n F(y_n)) - V(y_n) \leq \lambda \eta_n \dot{V}(y_n),
	\label{lyap_ineq_intro}
\end{equation}
where $\lambda \in \ ]0,1[\ $ is fixed and $\dot V$ denotes the (continuous) time-derivative of $V$ along the flow $F$. For GD, since $V=\R$ and $\dot V(\theta)=-\|\nR(\theta)\|^2$, condition \eqref{lyap_ineq_intro} boils down to the well-known Armijo condition
\begin{equation}
	\R(\theta_n-\eta_n \nR(\theta_n)) -\R(\theta_n) \leq -\lambda \eta_n \|\nR(\theta_n)\|^2.
	\label{armijo_full_intro}
\end{equation}
A backtracking linesearch (with a so-called {\em memory effect}, see \cite{Lyap_Theory_Bilel,Bilel_ICML,Bilel_thesis}) finds such an $\eta_n$ automatically, without any prior knowledge of the Lipschitz constant of $\nR$, and comes with guaranteed convergence toward critical points of $\R$, together with explicit convergence rates when $\R$ is analytic \cite{Lyap_Theory_Bilel,Bilel_thesis}.

Figures~\ref{fig_lc_motivation_a} and \ref{fig_lc_motivation_b} illustrate this automatic stabilization on the same \polyFive\ benchmark, for five of the six full-batch optimizers
of Figures~\ref{fig_speth_motivation_a}-\ref{fig_speth_motivation_b}. LC-AdamW is left out
since, to the best of our knowledge, AdamW does not admit a known Lyapunov function.
Some authors have even argued that such a function may not exist \cite{CuriousCase}.
The left column recalls the fixed-$\eta$ full-batch map, the right column shows its LC counterpart, started from an arbitrary initial learning rate. \\
The effect is often dramatic. For GD, the proportion of divergent trajectories collapses from $96.3\%$ to $0\%$ ($99.98\%$ now reach the global minimum); for RMSProp, from $1.6\%$
to $0.075\%$; for Momentum, from $68.1\%$ to $9.7\%$, even though a residual, scattered divergent area remains -- a signature that Lyapunov stabilization tempers Momentum's
instabilities but few more iterations may be needed for a greener map. Lion is roughly a wash ($12.8\%\to14.5\%$ divergent, $79.3\%\to84.0\%$ global): the two maps trade a few percent of divergence for a few percent of global minimum. \\
\textcolor{blue}{Adam is the instructive counter-example this time: LC-Adam is markedly {\em worse} than the hand-tuned, fixed-$\eta$ Adam on this stiff benchmark ($0\%\to33.6\%$
divergent, $79.1\%\to66.0\%$ global). Automatic learning rate selection is thus not a universal fix either: just as balancing alone could not rescue an ill-chosen constant $\eta$
for GD (Figure~\ref{fig_speth_motivation_a}), Lyapunov control alone can, on some optimizers, underperform a learning rate that has simply been tuned by hand. Both ingredients,
balancing and adaptivity, have their own blind spots.}

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.75\textwidth]{chapitre5_img/lc_motivation_poly5_a.png}
	\caption{Sensitivity maps on \polyFive\ for GD, Momentum and Adam (rows): full-batch with a fixed, hand-tuned learning rate (left, same maps as the left column of Figure~\ref{fig_speth_motivation_a}) versus its LC (Lyapunov-controlled) counterpart (right), with an automatically-adapted learning rate. Same color code as Figure~\ref{fig_speth_motivation_a}.}
	\label{fig_lc_motivation_a}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{chapitre5_img/lc_motivation_poly5_b.png}
	\caption{Sensitivity maps on \polyFive\ for RMSProp and Lion (rows), continuing Figure~\ref{fig_lc_motivation_a}: full-batch with a fixed learning rate (left) versus its LC counterpart (right). Same color code as Figure~\ref{fig_speth_motivation_a}.}
	\label{fig_lc_motivation_b}
\end{figure}

Building an adaptative strategy on top of the well-balanced mini-batch
          optimizers of \cite{partI} is the purpose of this paper: more precisely, we aim at combining the balanced splitting of \cite{partI}, which secures the {\em right} critical points in a mini-batch setting, with the Lyapunov-based automatic learning rate selection recalled above, which secures the learning rate {\em without tuning}, into a single mini-batch optimizer enjoying both properties.

This combination is, however, far from immediate. The ideal, full-batch counterpart of condition \eqref{lyap_ineq_intro} for the balanced update \eqref{RAG_scheme}, along the aggregator direction $g=g_n^i$, would read
\begin{equation}
	\R(\theta-\eta g) - \R(\theta) \leq -\lambda \eta \|g\|^2,
	\label{ideal_intro}
\end{equation}
but checking \eqref{ideal_intro} requires evaluating $\R$ itself, hence a full pass over the $P$ data, exactly the cost mini-batching is meant to avoid. A cheap, natural fallback consists in applying the Armijo condition batch by batch, using only $\R_i$ and $\nR_i$:
\begin{equation}
	\R_i(\theta-\eta \nR_i(\theta)) - \R_i(\theta) \leq -\lambda \eta \|\nR_i(\theta)\|^2,
	\label{batch_armijo_intro}
\end{equation}
which only costs the evaluation of one batch. But \eqref{batch_armijo_intro} does not guarantee \eqref{ideal_intro}: first, it certifies stability in the direction
$-\nR_i(\theta)$, whereas the balanced scheme actually moves along $g$, a different direction as soon as $m>1$; second, and more importantly outside of the interpolation regime, an
$\eta$ admissible for batch $i$ offers no guarantee whatsoever for batch $j\neq i$, since nothing prevents the batches from having arbitrarily different local stiffnesses, as was,
for instance, the case for the Momentum and Lion rows of Figures~\ref{fig_speth_motivation_a}-\ref{fig_speth_motivation_b}. Reconciling these two, seemingly incompatible
requirements, cheap, batch-local information on one side, and control of the true, full-batch dissipation on the other, is the crux of the construction presented in Section~\ref{section_optimizers}.
...
\section{Review on adaptive learning rate strategies}
\label{adapt}
~~\\
...
\begin{itemize}
	\item first there are attempts at extending the classical Armijo rule. S.Vaswani \cite{stochastic_armijo} suggested to apply the Armijo rule to each batch iteratively:
	\begin{equation*}
	...
	\item After Armijo rule comes the extension of trust regions to the stochastic case. Most of the generalizations necessitate to require strong assumptions on the noise
          ...
          %not conceivable 
          because, just as in the previous point, the strategy implies a progressive increase of the memory consumption. %Armijo. 
...
\end{itemize}
In the next section, we present our adaptive well-balanced strategy: it is also based on linesearches relative to the Armijo rule. 
...
In this section, we suggest a way to make the formulas \eqref{RAG_scheme}, \eqref{RAGL_splitting} put forward in \cite{partI} %and \eqref{RAGL2_splitting} 
...
prototype and RAGL\footnote{\newG{peut-etre que ce ne sera plus vrai plus tard...}}}
...
%\Speth-GD provides this information.  
...
%\paragraph{Identifying the stiffest batch, the $L_i$ etc. }
%~~\\
%
%Finally, the largest learning rate $\eta$ (between the two linesearches) is used to estimate the stiffness of the batch $i$: $L_i=\frac{2(1-\lambda)}{\eta}$. According to the descent lemma (proposition 1 of \cite{Lyap_Theory_Bilel}), this gives an estimation for the local Lipschitz constant of $\nR_i$ in the considered direction.
%
%
%The latter BWI is only called on the stiffest batch
...
The first algorithm we suggest is algo. \ref{algo_RAG}. 
It is called RAG. 
...
\begin{enumerate}
  \item the initialization procedure (Init\_RAG in algo. \ref{algo_RAG}) is described by algorithm \ref{algo_init}. 
	...
	$\nR_1,$ $\dots, \nR_m$ thanks to the linesearch without interaction. A possible estimation of the Lipschitz constant for $\nR$ is then obtained: $L=L_1+\dots+L_m$. 
        ...
        %\newG{Notice that in practice, we even use the majoration $\eta \leq 2L^{-1}$ (neglect the factor $1-\lambda$) because this choice empirically enables to save
        %time.\footnote{BEN C'EST PAS CE QYI EST DECRIT DANS L'ALGO? JE VIRE LA PHRASE.}}
	...
	Variable $R_0$, at line 6, corresponds to an approximation of $\R$ at the beginning of the epoch and $R$, at line 9, corresponds to its update.  
        The comparison between $R$ and $R_0$ at the end of the epoch enables to determine if the objective function diminished. %since these quantities are supposed to approximate it in the long run. 
	...
	\end{equation*}
	%\footnote{That is why we have tested SGD, RRGD and the different splittings with learning rates divided by $3=2\times 2-1$ in \cite{partI}.} 
        At line 33, the learning rate is again divided by a factor $h$ related to the heuristic described later in the paper. 
        %The goal is to activate the heuristic when its application conditions are verified at least twice in a row: \newG{the only aim of this line is to limit the potential oscillations generated by the heuristic}.
	...
	by the product $\frac{2(1-\lambda)}{\eta_1}\eta\|g\|$. Then this quantity gives the variations of the gradient of batch $i$ between two successive iterations. The variable
      ...
      \item Let us now details the heuristic relative to $h$ and $h_{max}$. Its aim is to accelerate the convergence of the algorithm in practice (it is possible to remove it to not take risks but the
        execution times are longer). If the estimation of $\R$ decreases during the epoch, we allow ourselves to be less restrictive on the "CFL" (i.e. stability) condition $\frac{2(1-\lambda)}{op\times L}$:
          ...
          %the iterates are "quasi mixed up" from the point of view of $\|g\|$. 
...
%\begin{algorithm}[h!]
%  \caption{RAG \newG{(en magenta: les trucs qui semblent manquer par rapport aux autres)}}
%	\label{algo_RAG}
%	\begin{algorithmic}[1]
...
The previously described algorithm is consequently adaptive and relies on updating the weights at each batch according to a well-balanced aggregator. 
...
%Due to its characteristics, it will be particularly adapted in the case in which the data can not fit into memory but the architecture can 

%\newG{ICI, J'AIMERAIS BIEN UNE BELLE PHRASE QUI MONTRE QUE C'EST PAS SI  GRAVE: SI ON A BEOSIN DE LA MEMOIRE POUR L'ARCHI ET LES M GRADIENTS, PAS GRAVE, ON MET MOINS EN MÉMOIRE ET L'ALGO ENCAISSE BIEN LES DESEQUILIBRE DE BATCHS. Mais au cas ou blabla... }

\paragraph{A first test to verify the behavior of the mini-batch optimizer with respect to its full batch counterpart}
\ \\
\newG{METTRE LES SENSITIVITY MAPS SUR POLYALLSTIFF DE LC-GD ET RAG: DIRE QU'ON A LES MEMES CHOSES = ON ESPERE PAS PLUS EN MINI-BATCH QU'EN FULL ET ON EST CONTENT.}
\newG{(et le fait d'introduire LC-GD plus tôt dans le discours, il sortira moins du chapeau plus tard)}

\subsection{{The RAGL-prototype optimizer: {\it Rebalanced Aggregated Gradient Light prototype}}}
\label{RAGL_proto}


\input{ALGO/RAGL_proto}
%\begin{algorithm}[h!]
...
In the previous section, a new adaptive mini-batch optimizer has been suggested\footnote{\newG{ICI, JE RÉDIGE COMME SI J'AVAIS LES SENSITIVITY MAPS DE LC-GD ET RAG L'UNE À COTE DE
L'AUTRE DANS LA SECTION PRECÉDENTE.}}. 
...
We now would like to reduce it. %cost of the previous adaptive well-balanced mini-batch optimizer. 
The mini-batch optimizer RAGL%-prototype 
...
that the adaptive procedures of the previous sections can easily be combined to 
 ...
 But first, we would like to emphasize the fact that a simpler is at hand ... \newG{ON POURRAIT METTRE \eqref{RAGL_splitting} and this would be deterministic RR-SARAH...} 
...
%\newG{Idealement, ici, on montre qu'on est content avec RAG. On montre qu'on perd des bonnes proprietes avec RAGL proto. On propose RAGL et on montre qu'on les retrouve... Voir si
%on peut faire ça en l'état, en remaniant... Ou s'il faut faire autre chose... Il faudrait limite 
%\begin{itemize}
%  \item la sensitivity map de LC-GD, RAG, RAGL-proto et RAGL sur PolyAllStiff (le cas du tableau). 
%  \item On pourrait rendre le truc plus progressif avec 
%    \begin{itemize}
%      \item Dans la section RAG: on montre LC-GD vs. RAG et on dit que c'est quasi pareil, c'esy assez fort mais surtout ça insiste sur le fait qu'on attend pas plus que d'avoir au
%        mieux les meêms perf qu'en full batch (en opposition a plein d'autres qui cherchent mystiquement à sortir d'un point selle ou d'un minimum global (ahah)
%      \item RAG vs. proto (qui est proche de SARAH): catastrophe, faut faire qqs chose... Et ça montrera bien que l'astuce de SARAH, existante, ne suffit pas.
%      \item RAG vs. RAGL: youpi
%    \end{itemize}
%      \item Et dans la suite, on ne regarde que LC-GD, RAG, RAGL (et le fait d'introduire LC-GD plus tôt dans le discours, il sortira moins du chapeau)
%    \end{itemize}
%    }

\paragraph{A first test to verify the behavior of the light mini-batch optimizer with respect to its full memory counterpart}
\ \\
\newG{PASSER POLYALLSTIFF AVEC RAGL-PROTO $\approx$ SARAH ET MONTRER QUE MEILLEUR EN MEMOIRE MAIS PAS SUFFISANT}
\ \\

\newG{Question pour améliorer les algo:
\begin{itemize}
  \item $eta_s$ n 'est ni $eta_1$ ni $eta_2$... qui ne sont pas initialisés quant à eux
  \item renommer $\eta_1$ par $\eta_{BFI}$ et l'autre $\eta_{BWI}$? 
  \item     A reussir a introduire (pas obligatoire mais si on peut le caser): si on avait acces aux $L_i$, on aurait moins besoin d'un algo adaptif. On pourrait avoir acces a un nouvel algo a pas constant... Mais comme on a un pas
    de temps, autant l'utiliser... N'empeche que si un jour on sait evaluer les $L_i$ autrement, on propose un algo a base de ces $L_i$. 
\end{itemize}
}
...
\begin{enumerate}
	...
	\item Lines 8-9 and 13-17 correspond to the scheme \eqref{RAGL_splitting}.
        \item By default, the number of operators is $4m-1$ (line 2 or 35). This is in fact an upper bound since at batch $i$, $2m+2i-1$ operators are involved\footnote{\newG{Trop
          raide ici je pense}}: the $m$ operators $\left(\nR_j\right)_{1\leq j \leq m}$ and the delays:
	\begin{multline*}
...
\ \\
\newG{SENSITIVITY MAPS: PASSER POLYALLSTIFF AVEC RAGL-PROTO $\approx$ SARAH ET MONTRER QUE MEILLEUR EN MEMOIRE MAIS PAS SUFFISANT}
\ \\
...
%In this section, we have introduced new adaptive mini-batch optimizers based on the balanced splitting schemes of section \ref{section_optimizers}.
...
\paragraph{One dimension benchmark (described in \cite{partI})}
~~\\
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Sonar_train.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Sonar_test.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Sonar_time.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Boston_grads.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Boston_train.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Boston_test.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Boston_time.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Diamonds_train.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Diamonds_test.pgf}}
...
\begin{figure}[h!]
	\centering
	\scalebox{0.7}{\input{chapitre5_img/Diamonds_time.pgf}}
