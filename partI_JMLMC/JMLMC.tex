...
locations which are more related to the chosen learning rate than the full batch loss $\R$, are recovered on at least $4$ of the benchmarks. The global minimumis totally ignored in
benchmark \exSeven (figure \ref{rr_adam_exs} (g)). 
\begin{figure}[h!]
	\centering
...
But there is an additional difficulty in build an eS-Adam mini-batch optimizer.  
Let us choose this simple splitting
$$
F = \sum_{b=1}^m F_b = \sum_{b=1}^m \frac{1}{m}F,
$$
which reuses Adam's update rule. The idea here is simply to build a consistent aggregator for the gradient and combine it with $F_b=\frac{1}{m}F$. 
The strategy is recalled in algorithm \ref{algo_speth_adam} (which can be applied to any optimizer, as will be verified in section \ref{mnist} with an application to Momentum).
Note that {\em as such, if we run the code, many divergences occur}. 
A closer look at some runs and initialisations lead to a loss of positivity of
the aggregator on $\nR^{\otimes 2}$. 


in order to show the flexibility of our approach, 

\textcolor{red}{Figure \ref{speth_adam_ex1_ex2}, times 30min40s vs. 21min44s., and grad**2 can be negative}


\ \\

        Note, once again, the variance reduction versions related to Adam, see Adam$^+$ \cite{adam_variance1}, are fundamentally different from this approach.
