...
\newG{The mini-batch optimizer eS-GD is studied in section \ref{eSGD} tpgether with its {\em low memory} counterpart. The combination with random batching is also tested. 
...
\newG{Finally, section \ref{mnist} focuses on a well-known high dimensional problem, the MNIST benchmark, in order to highlight the fact that the balanced splittings we suggest can
...
\newG{Figure \ref{RR_SPETH_exs} combines the extended Speth splitting with random batching (instead of deterministic incremental ones): the balancing, made by the
aggregator on the gradient, ensures, even in this stochastic context, to capture minimums of the full batch loss function. }

\newG{Finally, table \ref{ex1_table}... TODO}
...
%\begin{figure}[!h]
%	\centering
%	\rotatebox{90}{\quad \quad Adam}\scalebox{0.60}{\input{chapitre5_img/adam_exs.pgf}}
%	\rotatebox{90}{\quad \quad eS-Adam}\scalebox{0.60}{\input{chapitre5_img/speth_adam_exs.pgf}}
...
\SetKwInput{KwOutput}{Output}
  \KwInput{\newG{CE QU'ON A ÉCRIT} Training set $(X_{\text{train}},Y_{\text{train}})$, test set $(X_{\text{test}},Y_{\text{test}})$\\
...
\SetKwInput{KwInput}{Input}
\SetKwInput{KwOutput}{Output}
  \KwInput{\newG{CE QUE J'AI CODÉ} Training set $(X_{\text{train}},Y_{\text{train}})$, test set $(X_{\text{test}},Y_{\text{test}})$\\
         model hyper‑parameters: \texttt{max\_epochs}, \texttt{batch\_size}, learning rate $\eta$, tolerance $\varepsilon$}
\KwOutput{Trained network parameters $\theta$, test accuracy}
