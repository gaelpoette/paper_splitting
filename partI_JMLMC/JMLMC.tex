...
The previous construction allows stating
\begin{theorem}
\label{th}
...
Several comments can be made on the above theorem. First, we never rely on the interpolation condition. 
Optimizer is crucial, see condition $1$. Non unicity of the sum decomposition: some may be more or less usable in practice, see sections \ref{mom} and \ref{adam}.   
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{gPC coefficients \texttt{coeffs}, multi-indices \texttt{indices}}
\KwOut{First-order and total Sobol indices: \texttt{S\_i}, \texttt{S\_Ti}}

Compute total variance: 
\[
\texttt{total\_var} = \sum_{\boldsymbol{\alpha} \neq \mathbf{0}} c_{\boldsymbol{\alpha}}^2
\]

\ForEach{input variable $i$}{
    $A_i \gets$ indices where $\alpha_i \neq 0$ and all other $\alpha_j = 0$\;
    $T_i \gets$ indices where $\alpha_i \neq 0$\;

    $S_i[i] \gets \frac{1}{\texttt{total\_var}} \sum_{\boldsymbol{\alpha} \in A_i} c_{\boldsymbol{\alpha}}^2$\;
    $S_{T_i}[i] \gets \frac{1}{\texttt{total\_var}} \sum_{\boldsymbol{\alpha} \in T_i} c_{\boldsymbol{\alpha}}^2$\;
}
\Return{$S_i$, $S_{T_i}$}\;
\caption{Sobol Indices from gPC Coefficients}
\label{algo_speth}
\end{algorithm}
