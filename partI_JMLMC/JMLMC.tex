...
Of course here, care is taken to avoid the interpolary case (underparametrization).
...
\newG{Figure \ref{mnist_stats_2l} illustrates the training dynamics over $n_{\max} = 3000$ epochs on the MNIST benchmark, showing the evolution of loss (top left), accuracy (top right), and the final distributions of loss (bottom left) and accuracy (bottom right). The comparison includes four mini-batch strategies—RRGD, eS-GD, S-Momentum, and eS-Momentum—each run with $m = 5$ batches, a fixed learning rate $\eta = 10^{-4}$, and initialized with the same weights across 40 different seeds. The results once again highlight the advantages of momentum-based methods: eS-Momentum consistently reaches near-perfect accuracy with the fastest convergence and lowest variance, followed closely by S-Momentum. eS-GD also improves upon RRGD, approaching the performance of S-Momentum. By contrast, RRGD converges more slowly, exhibits higher variability, and results in significantly larger final losses.
}
%\begin{figure}[!h]
