...
\abstract{
  %250 mots max pour JMLMC
	Mini-batch optimizers are at the center of neural network training. 
        ...
        Here, we investigate the non-interpolating case and highlight some undesirable behaviors for the stationary distributions of optimizers. 
...
We prove that, without relying on the interpolation condition, the mini-batch optimizers we construct - if they do converge - converge to a critical point of the full-batch loss function.
    \item   Carefully chosen one-dimensional toy problems show that, as opposed to unbalanced ones, the proposed method avoids spurious stationary distributions and converges to true minima even in
...
%\begin{itemize}
%	\item the benchmark \polyTwo admits four global minima of zero values and four non-degenerate saddle points. It is the most simple configuration presented in \cite{Bilel}.
%	\item The benchmark \polyThree presents one global minimum of zero value and three local minima with different values. The saddle points are still non-degenerated but their values are differents.
...
\newG{By reducing the learning rate (see graphs b) and d)), the distribution becomes more continuous, but it remains noticeably off-centered from the global minimum.}
...
\newG{Unfortunately, SGD appears to favor interpolating minima regardless of their quality, even when the learning rate is small.}
...
Note that for SGD, even when decreasing the learning rate, the stationary distributions are not satisfactory: the Gaussian-like distribution is not well-centered (\exOne) or the interpolating local minimum is largely favored.

In this section, we built new benchmarks to analyze the influence of the interpolation condition on the long run behavior of SGD and RRGD. It appears that they have trouble
...
\end{itemize}
As a basis for splitting schemes, see \cite{splitting_ode_review,splitting_ode_review2}, and \cite{despres2022neural} for an introduction in a deep learning context.
...
\begin{equation*}
	...
	\right.
\end{equation*}
We recover the steady states of the simulations of figure \ref{IG_ex1} and notice that it is impossible to exactly obtain the minimum of $\R$ at $\theta=\dfrac{2}{5}$ if $\eta \neq 0$. \\
...
\begin{remark}
	...
	\begin{itemize}
		...
		\item For the composite problems where $\R=f+g$ with $f$ a differentiable function and $g$ a non-smooth convex function, there exists a literature about splitting methods \cite{splitting_proximal}. At first glance, no link was developed (until recently) between these methods and splitting schemes for ODE/PDE. The recent article \cite{splitting_proximal_ode} reinterprets the forward-backward, Douglas-Rachford and ADMM methods as splitting schemes on specific ODE.   
                \item \newG{Recently, in \cite{shaw2025} (a month after the publication of a first version of this work on HAL), the authors explore the link between stochastic gradient and randomized splitting strategies.}
	\end{itemize}
\end{remark}
...
This may be useful for instance if {\em a priori} knowledge of the batch stiffnesses is available. 
...
\label{red_mem}
~~\\
Now, the issue is that the implementation of formula \eqref{RAG_scheme} requires the storage of $m$ gradients hence its memory cost involves the product $mN$. Therefore, the goal is to obtain a \textbf{close approximation of $g_n^i$ while reducing the memory cost}.
%The previous algorithm has been described as if all the batches were in memory. 
The main ingredient of the previous algorithm consists in using an aggregator which \newG{is {\em consistent if not equal} to the full batch gradient}.
...
$$i_{\text{pivot}} = \argmax_{j\in\{1,...,m\}} \|F_j(y_{n-1+(j-1)/m})\|.$$}
This formula can be implemented with a reasonable memory cost of $3N$ instead of $mN$.
...
\end{enumerate}
 then $\theta_\infty$ is a critical point of the full batch function $\R$. 
\end{theorem}
The proofs can be found in \ref{proof}.\\
}

\newG{Several remarks can be made regarding theorem \ref{th}:
first, if convergence occurs in the mini-batch setting, it is toward a critical point of the full-batch objective $\R$, and this is achieved {\em without relying on the
...
Finally, {\em condition 2}, which pertains to the sub-operators in the splitting scheme, is absolutely not restrictive. 
Given a full-batch optimizer, multiple splitting schemes can be devised due to the non-uniqueness of the sum decomposition. Some of these schemes may be more practical or effective
than others - see section \ref{adam} for an illustrative example.}
...
Something important which seems, to our knowledge, to not have been noticed in the literature is that 
  ...
  Take the example of GD: in this case, $F_i=-\nR_i$ and $g_n^i = \nR(\theta_n)+\mathcal{O}(\eta)$. This means that we can choose to stop our optimizer as soon as:
  \begin{equation}
  \|g_{n+1}^m\|/P \leq\epsilon, 
  \label{stopping_criterion}
  \end{equation}
  with $\epsilon$ prescribed by the user, and this without resorting to a full batch evaluation of $\nR$. This will be numerically verified in section \ref{num_res}. \\
\ \\
...
satisfy condition 5 relative to the existence of a stopping criterion \eqref{stopping_criterion} which does not need a full batch estimation and condition 1 related to the fact of
...
\newG{Finally, section \ref{s_mnist} focuses on a well-known high dimensional problem, the MNIST benchmark, highlighting the fact that the balanced splittings we suggest can
...
For benchmarks \exOne and \exTwo, we used $\eta=\frac{1}{L_1+L_2}$, as the Lipshitz constants of $\nR_1,\nR_2$ are available: they correspond to 
...
On the last benchmark (figure \ref{SGD_exs} f)), neither the local nor the global minima are captured. 
...
convergent: with a  learning rate $100$ times smaller, $100$ times more epochs are needed to reach stationarity.
...
aggregator on the gradient, ensures, even in this stochastic context, to capture minima of the full batch loss function. On benchmark \exHeight, which is probably the stiffest
...
\begin{figure}[h!]
	\centering
	%\scalebox{0.60}{\input{chapitre5_img/DGD_exs_1.pgf}}
        \scalebox{0.90}{\includegraphics[width=\linewidth]{chapitre5_img/runtimes_gd.png}}
	\caption{Runtimes on the different examples}
	\label{ex1_table}
\end{figure}
...
\begin{figure}[h!]
	\centering
	%\scalebox{0.60}{\input{chapitre5_img/DGD_exs_1.pgf}}
        \scalebox{0.90}{\includegraphics[width=\linewidth]{chapitre5_img/runtimes_mom.png}}
...
catastrophic. The corresponding runtimes are shown in the last two columns of Figure \ref{ex1_example_mom}, where the eS-Momentum algorithm proves to be significantly faster --
$57$ times 
faster than S-Momentum on benchmark \exOne, and $37$ times faster on benchmark \exTwo.
}
...
\newG{The flow for Adam is given by \cite{barakat,Bilel,Bilel_thesis}
\begin{eqnarray}
  \label{Flow_adam}
...
\newG{Figure \ref{adam_fb_exs} displays the results on the 8 benchmarks of this section but with {\em full batch} Adam in exactly the same conditions as before.  
...
On another hand, figure \ref{rr_adam_exs} displays the results with RR-Adam, i.e. with the classical stochastic mini-batching. Similar comments as in section \ref{section_stationary} for
...
\begin{figure}[h!]
	\centering
	%\scalebox{0.60}{\input{chapitre5_img/adam_exs_1000.pgf}}
	%\scalebox{0.60}{\input{chapitre5_img/adam_exs_5000.pgf}}
	\scalebox{0.60}{\input{chapitre5_img/adam_exs_10000.pgf}}
        \caption{
...
\newG{Now, let us build operators $(F_i)_{i\in\{1,..,m\}}$ fulfilling {\em condition 2} in order to design an eS-Adam mini-batch optimizer.
...
F_b(\theta(t),v(t),s(t))=\left(\begin{array}{l}
  ...
  \end{array}\right).
\end{eqnarray}
\newG{As above, we have $\sum_{b=1}^m F_b=F$. But in the last line, the full batch gradient (or a consistent estimator of it) is necessary for this choice of sub-operators
$(F_i)_{i\in\{1,..,m\}}$.} 
...
\newG{Another option, illustrated here with $m=2$ batches for clarity, would be to choose:
\begin{eqnarray}
\label{F_j_adam_2}
F_1(\theta(t),v(t),s(t))=\left(\begin{array}{l}
  ...
  \end{array}\right),
\end{eqnarray}
from which the expression of $F_2$ can easily be deduced in order to fulfill condition 2 of theorem \ref{th}, i.e. $F=F_1+F_2$. 
...
\begin{figure}[h!]
	\centering
	%\scalebox{0.60}{\input{chapitre5_img/speth_adam_exs_1000.pgf}}
	%\scalebox{0.60}{\input{chapitre5_img/speth_adam_exs_5000.pgf}}
        %%\scalebex{0.60}{\input{chapitre5_img/speth_adam_exs_10000.pgf}}
	\scalebox{0.60}{\input{chapitre5_img/speth_adam_no_clip_exs.pgf}}
        \caption{
...
\newG{Note finally that choosing $F_b$ as
\begin{eqnarray}
\label{F_j_adam_3}
F_b(\theta(t),v(t),s(t))=\left(\begin{array}{l}
  \frac{1}{m}f(t) \frac{v(t)}{\sqrt{\epsilon_a+s(t)}}\\
...
\newG{Our aim now is to build a balanced mini-batch optimizer having Adam as a full batch one. 
...
The strategy is fully described\footnote{Note that in the previous case, factor $\frac{1}{m}$ is not necessary as stationarity is obtained up to a multiplicative constant in practice.} in algorithm \ref{algo_speth_adam} (which can be applied to any optimizer, as will be verified in section \ref{s_mnist} with an application to Momentum).
The numerical results are displayed in figure
...
%\textcolor{red}{Figure \ref{speth_adam_ex1_ex2}, times 30min40s vs. 21min44s., adam fb = 83m56,667s and grad**2 can be negative}
        Note, once again, that the variance reduction versions related to Adam, see Adam$^+$ \cite{adam_variance1}, are fundamentally different from the present approach.
...
\newG{
In this section, we compare the previously described splitting strategies to their classical stochastic mini-batch counterparts on the MNIST benchmark. 
...
\newG{Figure \ref{mnist} presents, for exactly the same initialization and exactly the same architecture, the loss (log scale) and accuracy curves along a training for RRGD, eS-GD, lm-eS-GD (left) and RR-Momentum,
eS-Momentum and lm-eS-Momentum (right). Each time, the learning rate is constant, chosen to $\eta = 10^{-2}$, and for Momentum $\beta_1 = 0.9$ is taken. 
...
Care is taken to avoid the interpolating regime (i.e., underparameterization), although in practice, verifying this condition remains challenging.
The number of batches is $m=5$. 
\begin{figure}[!h]
 ...
 \includegraphics[width=0.5\linewidth]{chapitre5_img/Momentum_accuracy.png}\\
  ...
  \label{mnist}
\end{figure}
In figure \ref{mnist} (left column), SGD, eS-GD and lm-eS-GD can be compared, in terms of both loss and accuracies across epochs. 
The eS-GD and lm-eS-GD display equivalent results while ensuring a gain of almost a decade at the final epoch when compared to SGD. 
}

\begin{figure}[!h]
 \includegraphics[width=0.8\linewidth]{chapitre5_img/128n_40s_loss.png}\\
% \includegraphics[width=0.5\linewidth]{chapitre5_img/128n_40s_final_loss_hist.png}\\
% \includegraphics[width=0.5\linewidth]{chapitre5_img/128n_40s_accuracy.png}
 \includegraphics[width=0.8\linewidth]{chapitre5_img/128n_40s_final_accuracy_hist.png}\\
  \caption{Loss curves (log scale on top  picture) across epochs and histograms of the final ($n_{\max}=1000$) accuracy (bottom picture) on MNIST for (exactly) the same initializations for RRGD, eS-GD, S-Momentum and
 eS-Momentum for a 1 hidden layer architecture of 128 neurons.}
  \label{mnist_stats}
\end{figure}
\newG{On this benchmark, it is noteworthy that the low-memory variant -- despite relying on a heuristic approach (as described in Section \ref{red_mem}) -- achieves performances comparable to that of eS-GD. The right column of Figure \ref{mnist} shows results obtained under identical conditions (including initialization) for S-Momentum, eS-Momentum, and lm-eS-Momentum. First, S-Momentum improves the loss by more than an order of magnitude compared to RRGD. Building on this, eS-Momentum further reduces the loss by over half an order of magnitude relative to S-Momentum.
}

\newG{In contrast, on this benchmark, lm-eS-Momentum performs similarly to S-Momentum. This outcome can be interpreted in several ways. One possibility is that the heuristic
introduced in Section \ref{red_mem}, which relies solely on gradient information and ignores momentum, may be inadequate for this particular task. However, if we assume that
lm-eS-Momentum is still capable of identifying non-interpolating critical points -- as observed in the 1D test cases -- then S-Momentum may also be capturing such points under these conditions. In that case, the results could suggest that the advantages of eS-Momentum extend beyond merely improving performance on non-interpolating landscapes. Naturally, in the absence of ground-truth information about the critical points on this benchmark, it remains difficult to determine which of these interpretations is accurate.
}

\begin{figure}[!h]
 \includegraphics[width=0.8\linewidth]{chapitre5_img/16_16_lr1e-4_loss.png}\\
% \includegraphics[width=0.5\linewidth]{chapitre5_img/16_16_lr1e-4_accuracy.png}\\
% \includegraphics[width=0.5\linewidth]{chapitre5_img/16_16_lr1e-4_final_loss_hist.png}
 \includegraphics[width=0.8\linewidth]{chapitre5_img/16_16_lr1e-4_final_accuracy_hist.png}\\
  \caption{Loss curves (log scale on top picture) across epochs and histograms of the final ($n_{\max}=3000$) accuracy (bottom picture) on MNIST for (exactly) the same initializations for RRGD, eS-GD, S-Momentum and
 eS-Momentum for a 2 hidden layers architecture of 16 neurons.}
  \label{mnist_stats_2l}
\end{figure}


\newG{Figure \ref{mnist_stats} presents results on the same benchmark under identical conditions, except with $n_{\max} = 1000$ and multiple training runs using different random
seeds (40 in total). First, the 40 runs per learning strategy highlight the statistical significance of the improvements achieved by the eS-based splitting methods. This effect is
particularly obvious in the top plot. The bottom plot shows the final epoch accuracies for each method: eS-GD demonstrates a marked improvement over RRGD. Notably, the best outcomes of eS-GD are comparable to the worst outcomes of S-Momentum, while the highest accuracies achieved by S-Momentum are among the lowest recorded for eS-Momentum.
}

\newG{Overall, these results suggest that eS-based strategies provide substantial empirical advantages over traditional mini-batch methods in real-world scenarios.}

\newG{Figure \ref{mnist_stats_2l} illustrates the training dynamics over $n_{\max} = 3000$ epochs on the same test-case but with a different architecture consisting in 2 layers of
16 neurons. 
Once again, the architecture is underparametrized to maximize the chances of being in the non-interpolary case.
The evolution of loss (top), and the final distributions accuracy (bottom) are displayed. The comparison includes the same four mini-batch strategies each run with $m = 5$ batches and a
{\em constant} learning rate $\eta = 10^{-4}$. %, and initialized with the same weights across 40 different seeds. 
The results once again highlight the edges of momentum-based methods: eS-Momentum consistently reaches near-perfect accuracy with the fastest convergence and lowest variance, followed closely by S-Momentum. eS-GD also improves upon RRGD, approaching the performance of S-Momentum. By contrast, RRGD converges more slowly, exhibits higher variability, and results in significantly larger final losses.
}
%\begin{figure}[!h]
...
machine-learning test similar to their full-batch counterparts. %This is the subject of part II.
...
%\bibliography{biblio_uniformized}   % name your BibTeX data base
...
%The values of the critical points $\theta^*$, in other words $\R(\theta^*)$, appear in the same order than the minima.
...
\section{Figures relative to the studies of sections 2 and 3}
...
\begin{figure}[h!]
	...
	%\scalebox{0.60}{\input{chapitre5_img/adam_exs_1000.pgf}}
	%\scalebox{0.60}{\input{chapitre5_img/adam_exs_5000.pgf}}
	\scalebox{0.60}{\input{chapitre5_img/adam_fb_exs.pgf}}
        \caption{
...
\section{\newG{Pseudo-code of the different new algorithms}}
\label{pseudo_code}
...
\SetKwInput{KwOutput}{Output}
  \KwInput{Training set $(X_{\text{train}},Y_{\text{train}})$, test set $(X_{\text{test}},Y_{\text{test}})$\\
         model hyper‑parameters: \texttt{max\_epochs}, \texttt{batch\_size}, learning rate $\eta$, tolerance $\varepsilon$, initial $y$}
...
\For{$\texttt{epochs}\gets 0$ \KwTo \texttt{max\_epochs}}{
    ...
    \ForEach{batch index $b$}{
        Extract mini‑batch $(x_b,y_b)$\;
        Forward passes: $z_b\leftarrow f_\theta(x_b)$,  $z_{\text{pivot}}\leftarrow f_\theta(x_\text{pivot})$\;
        $\ell_b\leftarrow\mathcal{L}(y_b,z_b), \ell_\text{pivot}\leftarrow\mathcal{L}(y_b,z_{\text{pivot}})$\;
        Back‑propagate gradients: $g_\text{pivot}\leftarrow\nabla_{\theta} \ell_b(\theta_\text{pivot})$, $g_b\leftarrow\nabla_\theta \ell_b(\theta)$\;
        Compute $F_b$ and $F_\text{pivot}$ from $y$ and $g_b$, $g_\text{pivot}$\;
        Agg \gets Agg - $F_\text{pivot}$ \tcc*{rm prev. contrib. }
        AggGrad \gets AggGrad - $g_\text{pivot}$ \tcc*{rm prev. contrib. for $\nabla R_b$}
        AggGrad \gets AggGrad + $g_b$  \tcc*{Update grad. aggregator}
        SumAgg \gets SumAgg + $F_b$  \tcc*{Update the sum }
        Agg \gets Agg + $F_b$ \tcc*{Update aggregator}
        \If{$L < \|F_b\|$}{ 
    ...
    Compute aggregator norm $\,\| AggGrad \|$\;
    \If{$\| AggGrad \|/P < \varepsilon$}{
        \textbf{break}\;  \tcc{converged}
    }
}
\BlankLine
...
\SetKwInput{KwOutput}{Output}
  \KwInput{Training set $(X_{\text{train}},Y_{\text{train}})$, test set $(X_{\text{test}},Y_{\text{test}})$\\
         model hyper‑parameters: \texttt{batch\_index} $b$, \texttt{learning rate} $\eta$, \texttt{current variable} $y$, $grad\_tab$ \texttt{current gradient table}, $AggGrad$ \texttt{current aggregator of the gradient}}
\KwOutput{Consistent approximation of the gradient $AggGrad$}
...
\BlankLine
        ...
        Back‑propagate gradients: $g_b\leftarrow\nabla_\theta \ell_b$\;
        \tcc{remove previous contribution for this batch}
        $AggGrad \leftarrow AggGrad - grad\_tab[b]$\;  
        \tcc{Update the gradient table}
        $grad\_tab[b]\leftarrow g_b$\;
        \tcc{Update aggregator}
        $AggGrad \leftarrow AggGrad + g_b$\;

\Return{$AggGrad$}
  \caption{Pseudo-code for the extended-Speth splitting procedure {\em building only an aggregator on the gradient} (as in sections \ref{adam}--\ref{mnist}) on an arbitrary full
  batch optimizer of flow $\dot{y}=F(y)=\sum_{i=1}^m F_i(y)$.}
...
\section{\newG{Proofs of the theorem}}
\label{proof}
\ \\
\newG{\textit{Proof for the extended Speth's splitting}}
\ \\ \ \\
\newG{Assume $(F_i)_{i\in\{1,...,m\}}$ as in theorem \ref{th} and consider first update \eqref{RAG_scheme} (extended Speth splitting) recalled here: 
\begin{equation}
...
\ \\
\ \\
\newG{\textit{Proof for the low memory version of the extended Speth's splitting}}
\ \\ \ \\
\newG{Assume once again that $(F_i)_{i\in\{1,...,m\}}$ are as in theorem \ref{th} and consider update \eqref{RAGL_splitting} (low-memory extended Speth splitting) recalled here: 
\begin{equation}
...
\end{equation}
which ends the proof assuming $\eta\neq 0$. 
}
