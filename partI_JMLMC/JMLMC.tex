...
\newG{Naturally, this choice is heuristic and inherently non-unique. However, it highlights the method's flexibility, relying only on minimal assumptionsi -- primarily consistency.
...
\begin{figure}[!h]
 \includegraphics[width=0.5\linewidth]{chapitre5_img/128n_40s_loss.png}
...
\begin{figure}[!h]
 \includegraphics[width=0.5\linewidth]{chapitre5_img/16_16_lr1e-4_loss.png}
 ...
 \includegraphics[width=0.5\linewidth]{chapitre5_img/16_16_lr1e-4_final_accuracy_hist.png}\\
  \caption{Loss curves (log scale on left picture) across epochs and histograms of the final ($n_{\max}=3000$) accuracy (right picture) on MNIST for (exactly) the same initializations for RRGD, eS-GD, S-Momentum and
 eS-Momentum for a 2 hidden layers architecture of 16 neurons.}
  \label{mnist_stats_2l}
\end{figure}


\newG{Figure \ref{mnist_stats} presents results on the same benchmark under identical conditions, except with $n_{\max} = 1000$ and multiple training runs using different random seeds (40 in total). First, the 40 runs per learning strategy highlight the statistical significance of the improvements achieved by the eS-based splitting methods. This effect is particularly evident in the left plot. The right plot shows the final epoch accuracies for each method: eS-GD demonstrates a marked improvement over RRGD. Notably, the best outcomes of eS-GD are comparable to the worst outcomes of S-Momentum, while the highest accuracies achieved by S-Momentum are among the lowest recorded for eS-Momentum.
}
...
\newG{Figure \ref{mnist_stats_2l} illustrates the training dynamics over $n_{\max} = 3000$ epochs on the MNIST benchmark, showing the evolution of loss (top left), accuracy (top
right), and the final distributions of loss (bottom left) and accuracy (bottom right). The comparison includes four mini-batch strategies -- RRGD, eS-GD, S-Momentum, and
eS-Momentum -- each run with $m = 5$ batches, a {\em constant} learning rate $\eta = 10^{-4}$, and initialized with the same weights across 40 different seeds. 
Once again, the architecture is underparametrized to maximize the chances of being in the non-interpolary case, with 2 layers of only 16 neurons.
The results once again highlight the advantages of momentum-based methods: eS-Momentum consistently reaches near-perfect accuracy with the fastest convergence and lowest variance, followed closely by S-Momentum. eS-GD also improves upon RRGD, approaching the performance of S-Momentum. By contrast, RRGD converges more slowly, exhibits higher variability, and results in significantly larger final losses.
}
%\begin{figure}[!h]
...
\For{$\texttt{epochs}\gets 0$ \KwTo \texttt{max\_epochs}}{
    ...
    \ForEach{batch index $b$}{
        ...
        Agg \gets Agg - $F_b(y,g_\text{pivot})$ \tcc*{rm prev. contrib. }
        AggGrad \gets AggGrad - $g_\text{pivot}$ \tcc*{rm prev. contrib. for $\nabla R_b$}
        AggGrad \gets AggGrad + $g_b$  \tcc*{Update grad. aggregator}
        SumAgg \gets SumAgg + $F_b(y,AggGrad)$  \tcc*{Update the sum }
        Agg \gets Agg + $F_b(y,AggGrad)$ \tcc*{Update aggregator}
        \If{$L < \|g_b\|$}{ 
        ...
        \tcc{Optimiser step with aggregated gradient}
        $y=(\theta,v) \leftarrow y(\theta,v) - \eta Agg$ \; %F_b(y,\,Agg)$\;
    }
    
    Compute aggregator norm $\,\| AggGrad \|$\;
    \If{$\| AggGrad \| < \varepsilon$}{
        \textbf{break}\;  \tcc{converged}
    }
}
\BlankLine
