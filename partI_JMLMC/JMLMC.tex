...
\begin{figure}[h!]
	...
	\scalebox{0.60}{\input{chapitre5_img/speth_adam_exs_10000.pgf}}
        \caption{
...
\For{$\texttt{epochs}\gets 0$ \KwTo \texttt{max\_epochs}}{
    ...
    \ForEach{batch index $b$}{
        ...
        Agg \gets Agg - $F_b(y,g_\text{pivot})$ \tcc*{rm prev. contrib. for $F_b$}
        AggGrad \gets AggGrad - $g_\text{pivot}$ \tcc*{rm prev. contrib. for $\nabla R_b$}
        AggGrad \gets AggGrad + $g_b$  \tcc*{Update $g_n$}
        SumAgg \gets SumAgg + $F_b(y,AggGrad)$  \tcc*{Update the sum of $F_n$}
