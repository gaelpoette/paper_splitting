...
% Use option "eqtion" for numbering equation as section
...
\abstract{
	Mini-batch optimizers are at the center of neural network training. 
        ...
        From this observation, a simple modification which balances them is proposed which leads to the construction of a new family of mini-batch optimizers based on classical ones.
}
...
\begin{itemize}
    ...
    \item   We develop a balanced update rule that aligns the fixed points of each sub-flow, eliminating the mismatch observed with standard methods. This framework provides, by construction, guarantees on the critical points obtained in a mini-batch setting and also enables the design of new full-batch optimizers.
    \item   Carefully chosen one-dimensional toy problems show that the proposed method avoids spurious stationary distributions and converges to the true minima even in non-interpolatory, imbalanced scenarios.
\end{itemize}
}
...
We expect their supports to be "centered" around the minima of $\R$. We will see that it is far from always being the case in the non-interpolating case. 
...
other words, several stationary states have nothing to do with the minima of the $\R_i$ or $\R$. By decreasing the learning rate (graphs b) and d)), we get closer to a Gaussian
...
\newG{In contrast, we adopt a different perspective: we interpret RRGD\textendash specifically, its deterministic variant\textendash as a particular discretization of the classical gradient flow}
...
~~\\
Let us remember that the deterministic version of RRGD is IG ({\it Incremental Gradient}) in the literature \cite{IG_proximal}. The latter can be interpreted as a Lie-Trotter
...
\begin{remark}
	...
	\begin{itemize}
		\item In \cite{splitting_sgd}, the authors interpret SGD (in fact RRGD) as a Lie-Trotter splitting. As an application, the same splitting is conserved by replacing the explicit Euler scheme by a Dormand\&Prince scheme (default integrator of the Scipy library). This process tested on linear and logistic regression problems improves the performances of SGD. However, there is no mention of the essential notion of stationary distribution and of balancedness. 
...
The main ingredient of the previous algorithm consists in using an aggregator which is consistent with the full batch gradient.
...
stiffest batch. We suggest: $i_{\text{pivot}}$ as
...
\begin{theorem}
\label{th}
Consider the flow of a given optimizer
$
\dot{y}(t) = F(y(t)).
$
Assume 
\begin{enumerate}
\item the flow is such that 
$
F(y)=0 \Longrightarrow \nR(\theta)=0,
$
\item we identified $m$ operators $(F_i)_{i\in\{1,..,m\}}$
such that $F=\sum_{i=1}^m F_i \newG{+\mathcal{O}(\eta)}$,
\item $(y_n)_{n\in\mathbb{N}}$ following the updates described in sections \ref{extendedSpeth}--\ref{red_mem} with operators $(F_i)_{i\in\{1,..,m\}}$ converges toward $y_\infty$. 
\end{enumerate}
Then $y_\infty$ is a critical points of $F$ (the full batch).
\end{theorem}
The proofs can be found in \ref{proof}.\\

Several remarks can be made regarding the theorem above:

First, if convergence occurs in the mini-batch setting, it is toward a critical point of the full-batch objective $F$, and this is achieved {\em without relying on the interpolation condition}.

Second, the choice of the underlying optimizer used in the splitting is crucial. If {\em Condition 1} is not satisfied (i.e., the full-batch optimizer does not lead to a critical point of $\R$), then its mini-batch variant cannot be expected to do so either.
...
The simplest choice for $F_i$ is $F_i=-\nR_i$, which is equivalent to IAG (SAG without random permutation) if memory reduction is not taken into account. % and RR-SARAH (without random permutation) otherwise. 
	...
	\end{equation}
        ...
        Momentum is indeed more stable than GD, at least on these benchmarks. 
        ...
        At epoch $n=20$, the (local and global) mimimums seem to be within the supports of the distributions, for both benchmarks. This 
...
\end{figure}
  ...
  In other words, the appearance of artificial minima strongly related to the choice of the learning rate emphasized in the previous examples does also occur for other optimisers
...
global minima of $\R$. A faster convergence rate is consequently numerically observed.  
  At epoch $n=2000$, for which stationarity is reached (in fact, the stopping criterion \eqref{stopping_criterion} involving $g_n^i$ is activated at $n\approx 40 - 60 \ll 2000$ depending on the
...
satisfies condition 5, relative to the existence of a stopping criterion \eqref{stopping_criterion} which does not need a full batch estimation, and condition 1 related to the fact of
...
\begin{table}[h!]
	\begin{center}
	\caption{Runtimes on the four first examples}
        \hspace{-3cm}\begin{tabular}{lllll}
%         Runtimes (s.)  & \exOne                &  \exTwo               & \exThree              & \exFour               & \exFive               & \exSix                & \exFive                  & \exSix\\ 
          Runtimes (s.) for & (a)                &  (b)              & (c)              & (d) \\ 
          RRGD           & $2.46 \times 10^{-0}$ & $2.40\times 10^{+1}$  & $6.83 \times 10^{+1}$ & $3.21 \times 10^{+1}$ \\ 
          eS-GD         & $7.28 \times 10^{-1}$ & $9.49\times 10^{-1}$  & $7.23 \times 10^{+1}$ & $1.32 \times 10^{+0}$ \\ 
          low memory eS-GD   & $8.56 \times 10^{-1}$ & $9.76\times 10^{-1}$  & $1.12 \times 10^{+2}$ & $4.04 \times 10^{+0}$ \\ 
      S-eS-GD         & $1.50 \times 10^{-0}$ & $2.00\times 10^{-0}$  & $1.12 \times 10^{+2}$ & $1.85 \times 10^{+0}$ \\ 
     RR-eS-GD         & $1.38 \times 10^{-0}$ & $1.81\times 10^{-0}$  & $1.27 \times 10^{+2}$ & $1.27 \times 10^{+0}$ \\ 
...
\begin{table}[h!]
	\begin{center}
	\caption{Runtimes on the four last examples}
	\hspace{-3cm}\begin{tabular}{lllllllll}
		%         Runtimes (s.)  & \exOne                &  \exTwo               & \exThree              & \exFour               & \exFive               & \exSix                & \exFive                  & \exSix\\ 
		Runtimes (s.) for & (e)               & (f)               & (g)                  & (h)\\ 
		RRGD & $4.26 \times 10^{+1}$ & $2.81\times 10^{+1}$& $6.20 \times
		10^{+1}$ & $5.14\times 10^{+1}$ \\ 
		eS-GD  & $1.32 \times 10^{+0}$ & $9.00\times 10^{-1}$& $2.14 \times  10^{+0}$ & $1.46\times 10^{-0}$ \\ 
		low memory eS-GD  & $1.68 \times 10^{+0}$ & $1.83\times 10^{+0}$& $3.70 \times 10^{+0}$ & $1.57\times 10^{+0}$\\ 
		S-eS-GD  & $1.30 \times 10^{+0}$ & $1.45\times 10^{-0}$& $3.61 \times  10^{+0}$ & $2.11\times 10^{-0}$ \\ 
		RR-eS-GD & $6.71 \times 10^{-1}$ & $8.20\times 10^{-1}$& $2.54 \times 10^{+0}$ & $1.31\times 10^{-0}$ \\ 
...
By tuning the learning rate for eS-GD, we mean having an estimation of the larger one we can use on the benchmarks in order to have stable resolutions. 
...
machine-learning test than their full-batch counterpart. %This is the subject of part II.
...
In this section, we gather the figures relative to the studies of sections \ref{section_stationary} and \ref{section_splitting_schemes} as we think it eases the readability of the paper. 
...
\section{\newG{Pseudo-code of the algorithms}}
\label{pseudo_code}
...
$\mathcal{L}\leftarrow$ Loss; $f(\theta, \cdot)$ (network output) \;
...
\For{$\texttt{epochs}\gets 1$ \KwTo \texttt{max\_epochs}}{
    ...
    \ForEach{batch index $b$}{
        ...
        Evaluate $F_b$ from $y$ and $g_b$\;
        \tcc{remove previous contribution for this batch}
        $Agg \leftarrow Agg - F\_tab[b]$\;  
...
\Return{$\theta$}
\caption{Pseudo-code for the extended-Speth splitting procedure (see section \ref{extendedSpeth}) on an arbitrary full batch optimizer of flow $\dot{y}=F(y)$.}
\label{algo_speth}
\end{algorithm}
...
%\tcc{Model definition}
%$\theta \gets$ \{Flatten$(28\times28)$, Dense$(128,\text{ReLU})$, Dense$(10,\text{softmax})$\}\;
$\mathcal{L}\leftarrow$ Loss; $Agg \gets 0$; $F\_tab[\;]\gets 0$; $\theta_{\text{pivot}} = \theta$ \;
%$\text{opt}\leftarrow$ SGD$(\eta)$\; (aggregator list matching $y=(\theta,v)^t$ depending on the used optimiser)\;
...
\For{$\texttt{epochs}\gets 1$ \KwTo \texttt{max\_epochs}}{
    ...
    \ForEach{batch index $b$}{
        Extract mini‑batch $(x_b,y_b)$\;
        Forward pass: $z_b\leftarrow f_\theta(x_b)$; $\ell_b\leftarrow\mathcal{L}(y_b,z_b)$\;
        Back‑propagate gradients at pivot   location: $g_\text{pivot}\leftarrow\nabla_{\theta} \ell_b(\theta_\text{pivot})$; Compute $F_\text{pivot}$\;
        Back‑propagate gradients at current location: $g_b\leftarrow\nabla_\theta \ell_b(\theta)$; Compute $F_b$ \;
        $Agg \leftarrow Agg - F_\text{pivot}$\;  
        $F\_tab[b]\leftarrow F_b$\;
        \tcc{Update the sum over the batches and the aggregator}
        SumAgg \gets SumAgg + $F$\_tab[b];
        $Agg \leftarrow Agg + F\_tab[b]$\;
        \tcc{Update the pivot weights} 
        \If{$L[b] < \|F_b\|$}{
           $\theta_\text{pivot} \leftarrow \theta$;
           $L[b] \leftarrow  \|F_b\|$
        }
        \If{b==number\_of\_batch}{
           Agg $\leftarrow$ SumAgg
        }
        $y\leftarrow y + \eta Agg$\;
    }
    \If{$\|Agg\| < \varepsilon$}{
...
\Return{$\theta$}
\BlankLine
\caption{Pseudo-code for the extended-Speth splitting procedure {\em with memory reduction} (see section \ref{red_mem}) on an arbitrary full batch optimizer of flow $\dot{y}=F(y)$.}
\label{algo_red_mem}
\end{algorithm}
