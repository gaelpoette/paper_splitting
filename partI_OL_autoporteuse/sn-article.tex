...
\item Let us now present variance reduction methods that do not need computation of $\nR$, tuning of a hyperparameter like $\tilde{m}$ or $\beta$, or the storage of $m$ gradients.
  The first method that fits into this setting is SCSG \cite{SCSG}. The update looks like the one for SARAH (we do not substract the evaluation of the last iterate but of the first
    ...
    are limited to the logistic regression (for only one initialization).  \newG{Finally, the interpolation condition, central in the present paper,
...
\end{figure}
  ...
  On the other hand, figure \ref{speth_mom_ex1_ex2} presents the results obtained with Speth-Momentum \newG{(with memory reduction)}, in exactly the same conditions as the ones of figure \ref{mom_ex1_ex2}. 
\begin{figure}[!h]
	\centering
...
In this section, we suggest applying the new previously described \newG{low memory} splitting combined to GD, i.e. {Speth-GD} to the 
