...
\subtitle{On the importance of well-balanced methods}
...
\begin{enumerate}
	...
	\item If possible, we do not want to store $m$ gradients. For example, the optimizers that do not need such a storage are:  SGD, RRGD, IG, SVRG, SNVRG, SPIDER, SARAH, SSRGD, PAGE, SCSG et RR-SARAH.
        ...
        \item Finally, the results of this paper will tend to show that one last ingredient for a user-friendly mini-batch optimizer would be an \textbf{adaptive learning rate method}. 
          ...
          In an adaptive context, all the methods of the litterature that do
...
criteria}. The last point will be tackled in further publications, based on the work described in this paper. We think these are crucial steps in order to develop smaller machine learning models. 
...
\end{figure}
  ...
  On the other hand, figure \ref{speth_mom_ex1_ex2} presents the results obtained with e-Speth-Momentum, for {\em extended Speth Momentum} {(without memory reduction)}, in exactly the same conditions as the ones of figure \ref{mom_ex1_ex2}. 
\begin{figure}[!h]
	\centering
	...
	\rotatebox{90}{\quad \quad epoch=2000}\scalebox{0.45}{\input{chapitre5_img/speth_mom_long_exs.pgf}}
        \caption{Stationary distributions of e-Speth-Momentum for learning rate $\eta=0.125$ on \exOne: a) and \exTwo: b), for $n_{max}=20$. The green straight line represents the position of the global minimum of $\R$.}
	\label{speth_mom_ex1_ex2}
\end{figure}
For the e-Speth-Momentum optimizer, stationarity is very close to being reached with only $20$ epochs: the supports of the obtained distributions are way tighter around the local and
global minimums of $\R$. A faster convergence rate is consequently numerically observed.  
  ...
  initialization), the e-Speth-Momentum algorithm recovers the 
...
{The substraction of $F_j$ evaluated at the point of epoch $n$, corresponding to the stiffest batch $F_j\left(y_{n-1+(i_{\max}-1)/m}\right)$ enables to suppress exactly the
...
stiffest batch. We suggest: $i_{\max}$ as
$$i_{\max} = \argmax_{j\in\{1,...,m\}} \|F_j(y_{n-1+(j-1)/m})\|.$$}
...
It is denoted as {e-Speth-GD} for {\em extended Speth GD}.
...
\begin{figure}[h!]
	\centering
	\scalebox{0.45}{\input{chapitre5_img/SPETH_RED_MEM_exs.pgf}}
        \caption{
          Application of low memory e-Speth-GD for the eight benchmarks depicted in section \ref{subsection_beyond_interpolation}, with $n_{max}=10000$ and
 $\eta = 1   \times 10^{-1}$  for \exOne    (a),
 $\eta = 5.81\times 10^{-3}$  for \exTwo    (b),
 $\eta = 1   \times 10^{-3}$  for \exThree  (c),
 $\eta = 3.33\times 10^{-2}$  for \exFour   (d),
...
Figure \ref{SPETH_exs} presents the results obtained with e-Speth-GD on the eight examples described in section \ref{subsection_beyond_interpolation}. 
...
$n_{max} = 10000$.  
Note that for e-Speth-GD, the final time always corresponds to the activation of the stopping criterion \eqref{stopping_criterion}, representative of the full gradient being smaller than $\epsilon =
...
epochs/batches.  
\ \\ \ \\
For the eight next benchmarks, the first step was to tune of the (constant) learning rate $\eta$ and the total number of epochs $n_{max}$ for e-Speth-GD (figure \ref{SPETH_exs}). 
By tuning the learning rate for e-Speth-GD, we mean having an estimation of the larger one we can use on the benchmarks in order to 
...
For all the other benchmarks, the larger learning rates have been (coarsely) manually obtained. They are given by 
 ...
 $\eta = 1   \times 10^{-2}$  for \exSeven  (g), 
 $\eta = 2.5 \times 10^{-2}$  for \exHeight (h). 
As can be seen on figure \ref{SPETH_exs}, the distributions obtained with e-Speth-GD are all peaked on the different local and global minimums, for the eight benchmarks, with
constant learning
rates which are tractable. 
%Note that on example \exHeight, figure \ref{SPETH_exs} (f), the e-Speth-GD optimiser does not find the global minimum and remains stuck on the local one: 
...
Figure \ref{SGD_exs} is obtained in the previous configurations of learning rates and maximum of epochs, the same as the ones of figure \ref{SPETH_exs} for e-Speth-GD, but with RRGD. 
...
It is the only hyperparameter to tune because we considered e-Speth-GD here. 
But other variations such as e-Speth-Momentum or e-Speth-Adam for example would have needed to tune other hyperparameters (classically denoted by $\beta_1,\beta_2, \epsilon_a$ in the litterature, see
...
>>>>>>> a2fb531fa1b854ed597d353745d6b22bd76ea0cb
