...
\abstract{
	Mini-batch optimizers are at the center of neural network training. 
        They are also called stochastic optimizers due to their intensive use of samplings within their implementations. 
        ...
        These behaviors are explained thanks to the interpretation of mini-batch optimizers as operator splitting.
        Furthermore, a simple modification which balances them is proposed which leads to the construction of a new family of mini-batch optimizers based on classical ones.
...
Therefore the goal of this paper and its following second part is to build a deterministic or stochastic (never mind) mini-batch optimizer that \textbf{satisfies the five first
...
\begin{itemize}
	...
	\item We want to be able to use different learning rates for every iteration (adaptive method in the end): $\eta^1$ and $\eta^2$. It is possible to keep the balance property in this context with some modifications, for example by replacing the division by $\eta$ by $\frac{2}{\eta^1+\eta^2}$. Nevertheless, exploiting the most recent evaluation (mentioned above), this issue is settled immediately.
        \item Moreover, the method shall be able to take any number of batches into account.
\end{itemize}
For these main reasons, in the following paragraph, we suggest an adjustment of Speth's splitting, convenient in our ML context. 
...
\begin{itemize}
	...
	\begin{equation*}
		...
		\begin{array}{ll}
			...
			y_{n+i/m} = y_{n+(i-1)/m}+\eta F(y_0) \text{ if } i=m.
		\end{array}
		\right.
	\end{equation*}
...
\newG{et cette histoire de $\theta_1$ qu'on garde? Elle est pas là?}
This formula can be implemented with a memory cost of $3N$.
...
\begin{figure}[h!]
	...
	\scalebox{0.45}{\input{chapitre5_img/SPETH_RED_MEM_exs.pgf}}
        \caption{
          Application of Speth-GD \newG{RED MEM avec le inter le batch avec le plus fort gradient} for the eight benchmarks depicted in section \ref{subsection_beyond_interpolation}, with $n_{max}=10000$ and
 $\eta = 1   \times 10^{-1}$  for \exOne    (a),
 $\eta = 5.81\times 10^{-3}$  for \exTwo    (b),
 $\eta = 1   \times 10^{-3}$  for \exThree  (c),
 $\eta = 3.33\times 10^{-2}$  for \exFour   (d),
...
in this case, decreasing the time step does help. It hints toward the fact that adaptivity may be necessary here in order to capture this stiff global minimum.  
Still, we will see in the descriptions of the behaviors of RRGD in the same conditions that the balanced splitting already provides significant benefits (with a way smaller cost
...
But other variations such as Speth-Momentum or Speth-Adam for example would have needed to tune other hyperparameters (classically denoted by $\beta_1,\beta_2, \epsilon_a$ in the litterature, see
