...
\begin{figure}[h!]
	...
	\scalebox{0.45}{\input{chapitre5_img/SPETH_RED_MEM_exs.pgf}}
        \caption{
          Application of low memory Speth-GD for the eight benchmarks depicted in section \ref{subsection_beyond_interpolation}, with $n_{max}=10000$ and
 $\eta = 1   \times 10^{-1}$  for \exOne    (a),
 $\eta = 5.81\times 10^{-3}$  for \exTwo    (b),
 $\eta = 1   \times 10^{-3}$  for \exThree  (c),
 $\eta = 3.33\times 10^{-2}$  for \exFour   (d),
...
%Note that on example \exHeight, figure \ref{SPETH_exs} (f), the Speth-GD optimiser does not find the global minimum and remains stuck on the local one: 
