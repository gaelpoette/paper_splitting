...
We respectfully disagree with the assessment that the paper offers only an incremental improvement over Speth {\it et al}'s work.
For instance, the aggregator $c_{n+1}$ (see (10)) is never explicitly computed in our algorithm.

Furthermore, in our view, Speth's contribution remains under-recognized, not only by the machine learning optimization community but even within the numerical analysis community to which it was originally addressed.\\
...
machine learning optimization literature (Speth {\it et all}'s paper is a good example). In fact, Reviewer \#2 noted that the literature review might even be too exhaustive for the scope of the paper.
...
Finally, we insist even more on what the introduction of Speth's splitting can bring to the ML optimization communities with more example (than extended Speth GD and extended Speth
Momentum). \\
...
In particular, we emphasize that this property naturally leads to the design of new (full-batch) optimization algorithms.

    In the new version, we tested the extended Speth strategy with random reshuffle. 
...
issues, or future plans to test on real-world tasks would help clarify the practical
    ...
    [TODO] Some benchmarks have been added, performance tests too. We hope they reach the reviewer's expectations.
...
be summarized in fewer plots with aggregate comparisons or referenced
        ...
        The new version should be close enough to what the reviewer suggests. 
    \end{itemize}
...
without requiring full gradient evaluations, as this is particularly relevant for the
      ...
      [TODO] This is now done, in blue, in the revised version. \\
    ...
    \item {\it The github link attached to the paper appears to be not available. Please fix this.}\\
      ...
      [TODO] {Sorry about that, it is a mistake. We changed the link. }\\
  \end{itemize} \ \\
...
\begin{itemize}
  \item {\it Lacks theoretical guarantees: no convergence or stability theorem is provided for the proposed method in nonlinear or high-dimensional settings.}
    ...
    Note that some of our benchmarks already were We are already nonlinear, even non-convex.  But it is true they were 1D. 
...
if only for display reasons. But results on MNIST benchmark have been added, as suggested.  
  ...
  \item {\it Extension to adaptive optimizers like Adam is only briefly discussed and not analyzed in depth.}
    ...
    [TODO] We added some detailed discussion on this point is the revised version. 
\end{itemize}
...
\begin{itemize}
  ...
  \item {\it Provide a formal (even local) convergence result for the balanced splitting scheme under reasonable assumptions.}
    ...
    We added an important property in the revised version, which does not rely on the interpolation condition. It concerns the convergence toward critical points. 
