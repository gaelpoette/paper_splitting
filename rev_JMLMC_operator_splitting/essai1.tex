...
\textcolor{blue}{[TODO]} In the revised version, we added another recent paper entitled "Randomised splitting methods and stochastic gradient descent" to complete the bibliography. 
...
issues, or future plans to test on real-world tasks would help clarify the practical
    ...
    [TODO] It is true the previous version of the paper focused on 1D problem and the reviewer definitely understood our illustrative intentions with them. 
...
\begin{itemize}
  ...
  \item {\it No benchmarking against variance reduction methods that also operate outside the interpolation regime, such as STORM or RR-SARAH.}
    ...
    \textcolor{blue}{[TODO] We regret that we did not have sufficient time to implement the STORM or RR-SARAH variance reduction methods. We hope this is acceptable, as we have
    made considerable efforts to address the reviewers' other suggestions and improve the manuscript. DIRE QU'ON A PAS PU LE FAIRE PARCE QUE TROP LONG et que c'est interessant mais
    que c'était pas l'interet
    premier du papier.}

  \item {\it It is unclear how robust the learning rate and stopping criteria are across diverse problem settings.}

    [TODO]  We hope this is now clearer with the runtimes discussion and the freshly added more realistic benchmarks. 
...
\begin{itemize}
  ...
  \item {\it Include clear pseudocode and computational cost analysis (both memory and FLOPs) for the proposed algorithm and its variants.}
    ...
    We have added pseudo-code and expanded our discussion of the memory implications associated with the complexity already analyzed in the first version. These additions have been
