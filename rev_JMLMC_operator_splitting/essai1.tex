...
in blue. In the following paragraphs, we stash together the common demands of all the reviewers and, in later paragraphs, address each of the reviewers' remarks individually.
...
[TODO] We are sorry the reviewer found the manuscript does not state clearly the scientific contribution. In the revised version, we insists more on it (in blue). 
Besides, we do not think the paper is only an incremental improvement of the original work of Speth. Speth's paper is, in our opinion, far from being acknowledged by the ML
optimization community. In our opinion, it is even not well enough acknowledged by the numerical analysis community to which Speth's paper is addressed. Our first intention with
this paper was to pedagogically explain and convince the need for balanced strategies. And we thought it could only be done {\em via} progressive well chosen simple test problems.
This, definitely, takes time and length. And reviewers \#2 and \#3 both agree with the fact that the paper is already long. We are sorry
the intention was not obvious enough for the reviewer. We hope it is better in the revised version.  \\ 
...
[TODO] It is true that the first version of the paper lacked multi-dimensional benchmarks. But we insist the 1D ones, which are (to our opinion and to the opinions of reviewers \#2 and
\#3) important to insist on the need for balancedness and visual and pedagogical. Still, some real-world benchmarks have been added, MNIST with MLP, as suggested by reviewers \#3.
Finally, we insist on the fact that we observed an undesirable behavior in 1D. It is true it is hard prooving it occurs in multi-D. But it is also hard prooving it does not appear
in multi-D. We think this 1D results may help the ML optimization
community have in mind this possibility and help in many ways. \\
