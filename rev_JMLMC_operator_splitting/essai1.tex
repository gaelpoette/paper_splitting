...
%\textcolor{blue}{[TODO] Vérifier que toutes les figures et les tables soient bien citées, vérifier s'il reste des ? dans le papier et des TODO dans ce doc + vérifier le format de la biblio et la bonne numérotation des paragraphes.}\\
...
In the revised version, we added another recent paper (available on arXiv since april, hence after our original submission) entitled "Randomised splitting methods and stochastic gradient descent" to complete the bibliography. 
...
This property does not need the interpolation condition, which, given our results, is certainly too restrictive. 
...
for exhaustiveness, some of them might be moved to an appendix or
        ...
        We moved the figures to the appendices while the comments on these figures remain in the body of the manuscript. 
...
be summarized in fewer plots with aggregate comparisons or referenced
        ...
        In the end, we opted for the last suggestion, namely some figures are moved to the appendices. 
...
without requiring full gradient evaluations, as this is particularly relevant for the
  ...
  remarks.\\
    ...
    \item {\it The github link attached to the paper appears to be not available. Please fix this.}\\
      ...
      {Sorry about that, it is a mistake. We changed the link. }\\
  \end{itemize} 
...
Of course, we would be happy to clarify any definitions or notations if the reviewer can indicate the specific points of concern.
  ...
  \item {\it The reference list is not consistent, many entries lack publication venues, or follow different formatting styles..}
    ...
    We checked the list of publications. We think it now respects a common formatting style. 
\end{itemize}
