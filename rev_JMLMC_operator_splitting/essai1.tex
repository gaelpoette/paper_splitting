...
For instance, the aggregator $c_{n+1}$ (see (10)) is never explicitely computed in our algorithm.
...
strengthen the paper and clarify the robustness of the method under realistic
    ...
    [TODO]    Thanks for this question. It appeared quite obvious to us but we thank the reviewer for giving the opportunity to add a property in the paper. 

    In fact, we tested the extended Speth strategy with random reshuffle. 
...
Note that it is easy not considering a reshuffle...
    ...
    [TODO] rajouter l'argument comme quoi si l'algo converge alors c'est vers un point critique.
...
\begin{itemize}
  \item {\it Lacks theoretical guarantees: no convergence or stability theorem is provided for the proposed method in nonlinear or high-dimensional settings.}
    ...
    We are already nonlinear, even non-convex.  
...
\begin{itemize}
  ...
  \item {\it Include clear pseudocode and computational cost analysis (both memory and FLOPs) for the proposed algorithm and its variants.}
    ...
    $+$ sur une iteration, pas de surcout significatif.
